...
(function() {
	...
	var HeroesOverview = View.extend({
		...
		registerViewComponents : function() {
			var _self = this, $el = this.$el, controller = this.controller, l10n = this.l10n;
			...
			var hero_id, hero, heroes = controller.getHeroes(), i , l = heroes.length, hero_of_town;
			...
			for (i = 0; i < l; i++) {
				...
				if (!hero.isAssignedToTown()) {
					controller.registerComponent('btn_hero_not_assigned_' + hero_id, $el.find('.btn_hero_not_assigned.' + hero_id).button({
						...
						tooltips : [
							{title : l10n.btn_assign_tooltip}, {title : l10n.btn_assign_tooltip_disabled}
						]
				...
				//When Hero is on the way town
					...
					controller.registerComponent('pb_on_the_way_town_' + hero_id, $el.find('.pb_on_the_way_town_' + hero_id).singleProgressbar({
						value : hero.getTransferToTownTimeLeft(), max : hero.getTransferToTownTime(), liveprogress : true, type : 'time',
						countdown : true, template : 'tpl_pb_single_nomax', caption: l10n.assignation
					...
					controller.registerComponent('btn_cancel_town_travel_' + hero_id, $el.find('.btn_cancel_town_travel.' + hero_id).button({
						tooltips : [{title : l10n.btn_cancel_town_travel_tooltip}]
				...
				//Unassigning
					...
					controller.registerComponent('btn_hero_unassign_' + hero_id, $el.find('.btn_hero_unassign.' + hero_id).button({
						caption : l10n.btn_unassign, tooltips : [{title : l10n.btn_unassign_tooltip}]
				...
				controller.registerComponent('level_change_indicator_' + hero.getId(), $el.find('.hero_slot.' + hero.getId() + ' .level').numberChangeIndicator({
					template: 'empty', caption : l10n.level(hero.getLevel())
				}));
			}
			...
			this.$el.find('.btn_recruit').each(function(index, el) {
				...
				controller.registerComponent('btn_recruit_' + index, $el.button({
					caption : l10n.btn_recruit, tooltips : [{title : l10n.btn_recruit_tooltip}]
            ...
            controller.registerComponent('dropdown_sort', $el.find('#dd_sort_heroes').dropdown({
                ...
                options: [
                    {value: 'level', name: l10n.hero_level_desc},
                    {value: 'origin_town_name', name: l10n.city_name_asc},
                    {value: 'name', name: l10n.hero_name_asc},
                    {value: 'category', name: l10n.hero_type}
                ]
		...
		createHeroCards : function() {
			var $el = this.$el, hero, hero_id, heroes = this.controller.getHeroes(), i, l = heroes.length;
			...
			for (i = 0; i < l; i++) {
				...
				$el.find('.hero_slot.' + hero_id + ' .portrait')
					.tooltip(TooltipFactory.getHeroCard(hero_id, {hero_level: hero.getLevel()}), {}, false);
			}
		},
