		...
		constructRevoltTooltipData : function(revolts, is_incoming) {
			...
			if (arising_revolt_count > 0) {
				...
				if (arising_revolt_count === 1) {
					player_name = is_incoming ? arising_revolt.getOriginTown().player_name : arising_revolt.getDestinationTown().player_name;
					arising_revolt_data.text = l10n_tooltips.revolts[direction].arising(this.getHumanReadableRevoltTime(arising_revolt), player_name);
				}
				if (arising_revolt_count > 1) {
			...
			if (running_revolt_count > 0) {
				...
				if (running_revolt_count === 1) {
					player_name = is_incoming ? running_revolt.getOriginTown().player_name : running_revolt.getDestinationTown().player_name;
					running_revolt_data.text = l10n_tooltips.revolts[direction].running(this.getHumanReadableRevoltTime(running_revolt), player_name);
				}
				if (running_revolt_count > 1) {
