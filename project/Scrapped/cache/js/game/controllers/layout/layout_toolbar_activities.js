	...
	var LayoutToolbarActivitiesController = GameControllers.BaseController.extend({
		...
		getTempleCommands: function () {
			return this.temple_commands.reduce(function (result, command) {
				...
				if (attacks_count > 0 || supports_count > 0) {
					result.push({
						temple_id: temple_id,
		...
		getFirstTimeout : function() {
			var trade, trades = this.getTrades(), i, l = trades.length, timeouts = [];
		 ...
		 * @return {String}
		 */
		getIncomingAttacksCommandsCountCaption : function(only_for_current_town) {
			var attacks_count = 0;
			if (only_for_current_town) {
				attacks_count = this.movements_units.getIncomingAttacksCount() + this.movements_revolts_defender.length;
			} else {
				attacks_count = CommandsHelper.getTotalCountOfIncomingAttacks();
			}
		...
		_getRecruitingQueueController : function($el, type) {
			var main_controller = this._getLayoutMainController(),
				context = {main : 'toolbar_activities_recruit', sub: type};
		...
		bindEventsOnTradesCollection : function() {
			var _self = this, collection = this.getTradesCollection(),
		...
		bindEventsOnUnitOrdersCollection : function() {
			var _self = this, collection = this.getUnitOrdersCollection(),
				//Function which updates list of options in the dropdown
				onCollectionChange = function() {
		...
		changeAttackIndicatorComponent : function() {
			var attack_indicator = this.getComponent('wgt_attacks');
			attack_indicator.setState(this.getIncomingAttacksCommandsCountCaption() > 0);
			attack_indicator.setCaption(this.getIncomingAttacksCommandsCountCaption());
		},


		changeCommandMovementIndicatorComponent : function() {
			var component = this.getComponent('btn_commands');
			component.setCaption(this.getCommandsCount(true, true));
		...
		bindEventsToPremiumFeaturesModel : function() {
			var _self = this, mdl_premium_features = this.getModel('premium_features');
