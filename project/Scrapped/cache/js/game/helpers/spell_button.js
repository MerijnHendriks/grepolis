...
TooltipFactory, Game, GameData, ITowns, MM */
	...
	return {
		...
		onConfirmCastAresSacrifice: function (on_confirm, town_id) {
			var population_available = ITowns.getTown(town_id).getLandUnits(true),
				population_destroyed = GameData.powers.ares_sacrifice.meta_defaults.amount,
			...
			if (current_fury + population_sum > max_possible_fury) {
				ConfirmationWindowFactory.openConfirmationWastedResources(
					function () {
						if (population_sum < population_destroyed) {
			...
			else if (population_sum < population_destroyed) {
				ConfirmationWindowFactory.openConfirmationAresSacrificeNotEnoughpopulation(on_confirm);
			}
			else {
				on_confirm();
