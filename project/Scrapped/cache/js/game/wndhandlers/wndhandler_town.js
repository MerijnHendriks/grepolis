/* global TownInfoHelper, WndHandlerTowns, ITowns, gpAjax, GPWindowMgr, Game, Espionage, DM, us, GameEvents, CM, GameData */
...
(function() {
	...
	var trade_action_callbacK = function(target_id, wnd_typeinforefid) {
		...
		var sp_wood = TradeTabData.spinners[target_id].wood, sp_stone = TradeTabData.spinners[target_id].stone, sp_iron = TradeTabData.spinners[target_id].iron,
			pb_wood = TradeTabData.progressbars[target_id].wood, pb_stone = TradeTabData.progressbars[target_id].stone,
			pb_iron = TradeTabData.progressbars[target_id].iron, pb_capacity = TradeTabData.progressbars[target_id].capacity,

			wood = sp_wood.getValue(), stone = sp_stone.getValue(), iron = sp_iron.getValue(),
			sent_resources = parseInt(wood, 10) + parseInt(stone, 10) + parseInt(iron, 10),

			i, l = wnds.length, handler;

		gpAjax.ajaxPost('town_info', 'trade', {id : target_id, wood : parseInt(wood, 10), stone : parseInt(stone, 10), iron : parseInt(iron, 10)}, false, function (data) {
			sp_wood.setValue(0);
			sp_stone.setValue(0);
			sp_iron.setValue(0);
			...
			for (i = 0; i < l; i++) {
				...
				if (wnds[i].typeinforefid !== wnd_typeinforefid) {
					...
					if (handler && handler.action === 'trading') {
						wnds[i].requestContentGet('town_info', 'trading', {id : handler.target_id});
					}
				}
			}
		}, {}, 'town_info_trade');
	...
	WndHandlerTown.prototype.onRcvData = function(data, controller, action) {
		...
		switch(action){
			...
			case 'trading':
				...
				if (sdata.resources) {
					var storage_volume = sdata.storage_volume, wood = sdata.resources.wood, stone = sdata.resources.stone, iron = sdata.resources.iron,
					inc_res = sdata.incoming_resources;

					//Initialize progressbars
					var pb_wood = TradeTabData.progressbars[target_id].wood = root.find('#town_capacity_wood').progressbar({max : storage_volume, value : wood, value2 : inc_res.wood, value3 : 0}),
					pb_stone = TradeTabData.progressbars[target_id].stone = root.find('#town_capacity_stone').progressbar({max : storage_volume, value : stone, value2 : inc_res.stone, value3 : 0}),
					pb_iron = TradeTabData.progressbars[target_id].iron = root.find('#town_capacity_iron').progressbar({max : storage_volume, value : iron, value2 : inc_res.iron, value3 : 0});
				}

				//Initialize spinners
				TradeTabData.spinners[target_id].wood = root.find('#trade_type_wood').spinner({
					value : 0, step : 500, max : origin_wood, tabindex : 51
				}).bind('sp:change:value', function(e, new_val, old_val) {
				...
				TradeTabData.spinners[target_id].stone = root.find('#trade_type_stone').spinner({
					value : 0, step : 500, max : origin_stone, tabindex : 52
				}).bind('sp:change:value', function(e, new_val, old_val) {
					pb_capacity.decr(new_val - old_val);
				...
				TradeTabData.spinners[target_id].iron = root.find('#trade_type_iron').spinner({
					value : 0, step : 500, max : origin_iron, tabindex : 53
				}).bind('sp:change:value', function(e, new_val, old_val, arg) {
					pb_capacity.decr(new_val - old_val);
				...
				root.find('.content').unbind('click').bind('click', function(e) {
					var target = $(e.target);
			...
			default:
				...
				root.find('a.town_bbcode_link').bind('click',function() {
					root.find('input.town_bbcode_id').toggle().focus();
				});
				break;
		}
	};
	...
	WndHandlerTown.prototype.getPage = function(arg1, arg2){
		this.wnd.requestContentGet('town_info', arg1, {id: arg2});
		return null;
	};
	...
	WndHandlerTown.prototype.inviteIntoAlliance = function(player_name) {
		this.wnd.ajaxRequestPost('alliance', 'invite', {player_name : player_name}, function(_wnd, return_data) {
	...
	WndHandlerTown.prototype.handleEvents = function(event, that) {
		...
		if (event.type === GameEvents.town.town_switch /*|| event.type === GameEvents.town.hide.change*/){
			...
			this.wnd.requestContentGet('town_info', this.action, {'id':this.target_id});
		}
	};
