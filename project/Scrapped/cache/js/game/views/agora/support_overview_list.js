...
(function() {
	...
	var SupportOverviewListView = BaseView.extend({
		...
		render : function() {
			var controller = this.controller, mode = controller.getMode(), modes = controller.getModes(),
				num_of_supports = controller.getSupportsCount();

			var are_units_in_town = controller.getUnitsInTown(), is_more_than_one_support = num_of_supports > 1, no_supports = num_of_supports === 0;
		...
		renderSupports : function() {
			var controller = this.controller, supports = controller.getSupports(),
				i, l = supports.length, support, is_even = true;
