...
(function() {
	...
	WndHandlerUninhabited.prototype.render = function(){
		...
		this.root.find('a.select_all_units').bind('click', function() {
		...
		this.root.find('input.unit_input').bind('keyup change', function() {
			that.bindDurationCounter();
			that.bindCapacityCounter();
		});
	...
	WndHandlerUninhabited.prototype.onRcvData = function(data){
		...
		if(data.tmpl) {
			GameData.add({'ColonizeTemplate' : data.tmpl});
		}
	...
	WndHandlerUninhabited.prototype.sendColonizer = function() {
		...
		if (!is_one_colonize_ship_selected) {
			this.wnd.ajaxRequestPost('uninhabited_place_info', 'send_colonizer', this.params, function(data){
