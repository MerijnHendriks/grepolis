...
(function() {
	...
	var AlliancePact = {
		...
		showGoBackButton : function(current_aliance_id) {
			var l = this.pageHistory.length, last_row = this.pageHistory[l - 1];
			return l > 0 || (last_row && last_row.id !== current_aliance_id);
		},

		joinPact: function(inv) {
			gpAjax.ajaxPost('alliance', 'join_pact', {invitation_id: inv},true, function(data) {
		...
		cancelInvitation: function(inv) {
			gpAjax.ajaxPost('alliance', 'cancel_pact_invitation', {invitation_id: inv},true, function(data) {
				$('#invitation_' + data.invitation_id).remove();
			});
		},

		cancelPact: function(id) {
			gpAjax.ajaxPost('alliance', 'cancel_pact', {pact_id: id},true, function(data) {
		...
		mark_enemy: function() {
			...
			gpAjax.ajaxPost('alliance', 'mark_enemy', {name:name},true, function(data) {
		...
		end_enmity: function(enmity_id) {
			gpAjax.ajaxPost('alliance', 'end_enmity', {enmity_id:enmity_id},true, function(data) {
