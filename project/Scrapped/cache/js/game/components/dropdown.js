...
(function($) {
	...
	$.fn.dropdown = function (params) {
		...
		var _self = this,
			initial_width,
			//HTML elements
			$el = $(this), $list, $caption, $tpl, $empty, $close_list_layer,
		...
		function createCloseListLayer() {
			var attr = {
				position : 'absolute', top : 0, left : 0, right : 0, bottom : 0, zIndex : list_z_index
			};
		...
		function isInOptions(value) {
			var options = settings.options, i, l = options.length;
		...
		function bindEvents() {
			...
			$list.on('click.dropdown', '.option', function(e) {
				...
				var exclude_click_nodes = settings.exclude_click_nodes,
					i, l = exclude_click_nodes.length, set_value = true,
					hide_on_click = true;
				...
				if (!isExcluded(value)) {
					if (set_value) {
						_self.setValue(value, {force : settings.repeatable_selection});
					}
		...
		function callOnInit() {
			...
			$list.css({display : 'block', left : 50000});

			if (typeof settings.onInit === 'function') {
				settings.onInit.call(_self, $list);
			}

			$list.css({display : start_display, left : start_pos.left});
		}
		...
		function selectOption(value) {
			var $option, type = settings.type, option_value,
				not_excluded, set_different_val = false, caption;
			...
			if (set_different_val) {
				_self.setValue(not_excluded, {silent : true});
			}
			else {
				if (!isInOptions(value)) {
					_self.setValue(settings.default_value, {not_existing_value : true});
				}
			}
		}
		...
		function updateDropDownListPosition() {
			var width = $el.outerWidth(true), height = $el.outerHeight(true),
				list_pos = settings.list_pos,
                list_top_pos_pixel = settings.list_top_pos_pixel,
				offset = $el.offset(),
				mod_top = height - 1 + list_top_pos_pixel, mod_left = 0,
				list_height, list_width, list_left, list_top, list_bottom, list_right;
			...
			var d_width = $document.outerWidth(), d_height = $document.outerHeight();

			//Lest skip checking whether the list is cut on the top, because list
			//opens always below the "dropdown" element, so technicaly it can not happend
			...
			if (list_bottom > d_height) {
				//Check if its possible to move it above the dropdown
				if (offset.top - list_height > 0) {
					list_top = offset.top - list_height;
				}
				//Just put it on the bottom border
					...
					// TODO rewrite the line into a more easely way
		...
		/**
		 * Makes dropdown list element visibile or not, depends on the previous state.
