...
(function($) {
	...
	$.fn.datepicker = function(params) {
		var settings = $.extend({
			steps : {
				day : 1, month : 1, year : 1, time : '00:10:00'
			},
			timestamp : null,
			disabled : false
		}, params);
		...
		function updateComponentsValues(time_values) {
			$sp_day.setValue(time_values.day, {silent : true});
			$sp_month.setValue(time_values.month, {silent : true});
			$sp_year.setValue(time_values.year, {silent : true});
		}
		...
		(function() {
			...
			$sp_day = $el.find('.js-spinner-day').spinner({
				min : 1, max : 31, value : date_values.day, step : steps.day, tabindex : 1
			...
			$sp_month = $el.find('.js-spinner-month').spinner({
				min : 1, max : 12, value : date_values.month, step : steps.month, tabindex : 2
			}).on('sp:change:value', function(e, new_val, old_val, _sp) {
				var time_values = recalculateDateValues();
				updateComponentsValues(time_values);
			});
			...
			$sp_year = $el.find('.js-spinner-year').spinner({
				min : 2012, max : 2200, value : date_values.year, step : steps.year, tabindex : 3
			}).on('sp:change:value', function(e, new_val, old_val, _sp) {
				var time_values = recalculateDateValues();
				updateComponentsValues(time_values);
			});
			...
			$sp_time = $el.find('.js-spinner-time').spinner({
				min : '00:00:00', max : '24:00:00', value : getTimeValue(objDate), step : steps.time,  type : 'time', tabindex : 4
			});
