	...
	if (Game.dev || InternalMarketsHelper.isInternalMarket(Game.market_id)) {
		window.DebugTimeOffset = (function() {
			...
			function setMinutesUntilMidnight(minutes) {
				gpAjax.ajaxPost('debug', 'macro', {macro : 'dailylogingift_setminutesuntilmidnight', args : {minutes : minutes}}, false, function() {
					window.location.reload();
				});
			}

			function reset() {
				gpAjax.ajaxPost('debug', 'macro', {macro : 'dailylogingift_resetdebugtimeoffset'}, false, function() {
					window.location.reload();
				});
			}
