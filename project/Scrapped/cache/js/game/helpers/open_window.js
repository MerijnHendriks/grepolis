	...
	var hWindow = {
		...
		openReservationList : function(reservation_id, callback) {
			...
			hCommon.openWindow(GPWindowMgr.TYPE_ALLIANCE, _('Reservations'), {
				noInitRequest : true
			}, 'reservation', 'index', {reservation_id : reservation_id}, 'get', callback);
		},
		...
		openActivateAdvisorWindow : function(advisor_id) {
			...
			hCommon.openWindow(GPWindowMgr.TYPE_DIALOG, _('Activate the premium adviser'), {prevent_default_request : true}, 'premium_features', 'advisor_activate', {
				advisor : advisor_id,
		...
		openMentoringInfoWindow : function (action) {
			...
			hCommon.openWindow(GPWindowMgr.TYPE_INVITE_FRIENDS, _('Invite players'), {prevent_default_request: true}, 'invite_friends', action, {}, 'get');
		},
		...
		showConfirmDialog : function(title, text, onConfirmation, confirm_text, onCancel, cancel_text, onCheck, check_text, css_class, use_player_hint) {
			...
			wnd = GPWindowMgr.Create(GPWindowMgr.TYPE_CONFIRM_DIALOG, title, {modal: false, css_class : css_class});
			if (wnd) {
		...
		openPremiumOverviewWindow : function(sub_tab) {
			...
			if (sub_tab) {
				params = {
					sub_content : 'premium_overview', sub_tab : sub_tab
				};
			}
		...
		viewAttackPlan : function(attack_id) {
			...
			if (w) {
				...
				GPWindowMgr.Create(GPWindowMgr.TYPE_ATTACK_PLANER, _('Attack planner'), {}, 'attacks', {attack_id : attack_id});
			}
		}
	};
