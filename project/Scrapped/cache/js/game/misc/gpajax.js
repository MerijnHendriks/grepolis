...
function GPAjax(layout, locking, class_options) {
	 ...
	 */
	window.request_stacks = window.request_stacks || {current: undefined, waiting: undefined};
	...
	function _doGpajaxrequest(controller, action, params, type, callback, show_loader, http_status_code_options, town_id) {
		...
		if (window.MaintenanceWindowFactory && MaintenanceWindowFactory.isMaintenanceTime()) {
			return; // don't do anything when maintence (user logged out)
		}
		...
		var jqXHR = jQuery.ajax({
			url: url(controller, action, {town_id: town_id}),
			type: type,
			data: {'json': JSON.stringify(stripped_params)},
			dataType: 'text',
			...
			success: function(data, flag, jqXHR) {
				...
				var return_data = (data && data.length) ? $.parseJSON(data) : null;
