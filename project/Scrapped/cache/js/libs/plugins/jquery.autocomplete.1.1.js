...
$.Autocompleter = function(input, options) {
	...
	$input.bind('blur.autocomplete', function() {
		hasFocus = 0;
	...
	$input.bind('click.autocomplete', function() {
		// show select when clicking in a focused field
		if ( hasFocus++ > 1 && !select.visible() ) {
			onChange(0, true);
		}
	});
	$input.bind('search.autocomplete', function(arg1, arg2) {
	...
	$input.bind('flushCache.autocomplete', function() {
		cache.flush();
	});

	$input.bind('setOptions.autocomplete', function() {
		$.extend(options, arguments[1]);
		// if we've updated the data, repopulate
		if ( "data" in arguments[1] )
			cache.populate();
	});
	$input.bind('unautocomplete.autocomplete', function() {
		select.unbind();
		$input.unbind('.autocomplete');
		$(input.form).unbind(".autocomplete");
	});

	$input.bind('hide-list.autocomplete', function() {
		hideResults();
	});
