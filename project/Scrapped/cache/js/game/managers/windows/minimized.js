	...
	views.ViewMinimizedWindows = Backbone.View.extend({
		...
		render : function() {
			var $minimized = this.$minimized,
				...
				$boxes,
				total_width = 0, max_size, overloaded;
		...
		toggleMinimizedWindowsArea : function() {
			var _self = this, collection = this.collection;
			...
			function toggleMinimizedArea() {
				...
				//console.log(_self.$btn_close_all.length);
				_self.$btn_close_all[_self.open_windows.getOpenedClosableWindows().length + GPWindowMgr.getOpenedClosableWindows().length > 0 ? 'show' : 'hide']();
		...
		_getScrollView : function(item_nr) {
			var $windows = this.$windows, $windows_box = this.$windows_box,
				$boxes = $windows.find('.box');

			var viewport_size = $windows_box.outerWidth(),
				scroll_to, total_size = 0, curr_position = 0, temp_size = 0,
				edge_item_nr = null;
			...
			$windows.addClass('scrolling').css({left : -scroll_to});
		},
		...
		_removeMinimizedWindow : function(wnd_model) {
			var collection = this.collection,
				model = collection.where({wnd_cid : wnd_model.cid, type : 'new'});
		...
		_removeMinimizedWindowForOldWindows : function(window_id) {
			var collection = this.collection,
				model = collection.where({wnd_cid : window_id, type : 'old'});
		...
		_addMinimizedWindow : function(wnd_model) {
			...
			this.collection.add([{wnd_cid : wnd_model.getIdentifier(), title : wnd_model.getTitle(), type : 'new', closable : wnd_model.isClosable() }]);
			$.Observer(GameEvents.window.minimize).publish({window_obj : wnd_model});
		},

		_addMinimizedWindowForOldWindows : function(wnd) {
			//Add information about old minimized window
			this.collection.add([{wnd_cid : wnd.getIdentifier(), title : wnd.getTitle(), type : 'old', closable : wnd.isClosable()}]);
			$.Observer(GameEvents.window.minimize).publish({window_obj : wnd});
		},
