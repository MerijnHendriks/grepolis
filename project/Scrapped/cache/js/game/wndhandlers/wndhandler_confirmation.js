...
(function() {
	...
	WndHandlerConfirmation.prototype.initialize = {
		confirm : function(ctx) {
			var root = ctx.wnd.getJQElement(), cnt = ctx.cnt,
				settings = ctx.settings,
				lang = settings.lang;

			//Initialize buttons
			cnt.btn_decision_confirm = root.find("#btn_decision_confirm").button({caption : lang.btn_confirm}).on("btn:click", settings.onConfirm);
			cnt.btn_decision_cancel = root.find("#btn_decision_cancel").button({caption : lang.btn_cancel}).on("btn:click", settings.onCancel);

			//Initialize checkbox
			cnt.cbx_hide_confirmation_window = root.find("#cbx_hide_confirmation_window").checkbox({caption : lang.cbx_caption}).on("cbx:check", settings.onCheck);
		},
		...
		buy_gold : function(ctx) {
			...
			cnt.btn_buy_gold = root.find("#btn_buy_gold").button({
				caption : lang.btn_caption, template : 'tpl_large_button'
