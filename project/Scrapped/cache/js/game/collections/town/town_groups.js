...
define("collections/town/town_groups", function() {
	...
	TownGroups.getTownGroups = function() {
		var group_id, group, groups = this.models.clone(), i, l = groups.length,
			sorted = [], all_towns_group, no_group_group, sorter = new StringSorter();
		...
		sorted = sorter.compareObjectsByFunction(sorted, function(group) {return group.getName();});
	...
	TownGroups.getTownGroupsForDropdown = function() {
		var town_groups = this.getTownGroups(), town_group,
			i, l = town_groups.length, options = [];
		...
		for (i = 0; i < l; i++) {
			...
			if (town_group.getId() !== NO_GROUP_GROUP_ID) {
				options.push({value : town_group.getId(), name : town_group.getName()});
			}
		}
	...
	TownGroups.triggerCollapsedChange = function(state) {
		this.changed = {collapsed: state}; // backbone internal stuff
