...
(function() {
	...
	function MassRecruitTown(data, gods_favor) {
		...
		us.each(data.casted_power, function(item, key) {
			...
			data.extended_casted_power[key] = GameDataPowers.getTooltipPowerData(GameData.powers[item.power_id], $.extend({casted_power_end_at: item.end_at}, configuration));
		});
	...
	MassRecruitTown.prototype._getMaxBuildOfUnits = function(itown) {
		var unit, unit_id, units = this.getUnits(),
			res = itown.resources(), max_build, gd_unit, unit_max_build,
			max = {}, factor;
				...
				if (gd_unit.id !== 'militia') {
					...
					if (gd_unit.favor) {
						...
						if (this.hasGod() && ((this.getGod() === gd_unit.god_id) || gd_unit.god_id === 'all')) {
							...
							max_build[max_build.length] = this.gods_favor.getCurrentFavorForGod(gd_unit.god_id, this.getGod()) / gd_unit.favor;
						}
					}
	...
	MassRecruitTown.prototype.updateUnits = function(units) {
		var i, l = units.length, unit, objUnit;
