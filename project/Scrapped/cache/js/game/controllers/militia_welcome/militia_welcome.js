/*global ITowns, GameControllers, GameViews, HelperPlayerHints, BuildingWindowFactory */
...
(function() {
	...
	var MilitiaWelcomeController = GameControllers.TabController.extend({
		...
		onButtonClick : function() {
			...
			//Disable info window
			HelperPlayerHints.disable('militia');

			var militia_town = ITowns.getTown(militia_town_id);

			//There can be situation when militia window is opened for longer time and the town is already conquered and does not belong to the user anymore
			//In this case window should be closed GP-13234
			if (militia_town !== undefined) {
