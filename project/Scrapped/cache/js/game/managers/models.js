...
(function() {
		...
		if (modelCollections) { // Is there a collection for this model class available
			for (idx in modelCollections) {
				if (modelCollections.hasOwnProperty(idx)) {
					...
					modelCollections[idx].add(model, {silent: silent, update_emergence_time : update_emergence_time});
				}
			}
		}
	};
		...
		for (backend_model_name in json) {
			if (json.hasOwnProperty(backend_model_name)) {
				...
				if (backend_model_name === 'DeletedModelNotification') {
					this.deletedModelNotification(modelJson, {update_emergence_time : notification.time});
				} else {
		...
		for (object_id in object_specs) {
			if (object_specs.hasOwnProperty(object_id)) {
				if (creation_arguments) {
					options = {creation_arguments: creation_arguments};
				} else {
					options = {};
				}
						...
						if (class_container[object_id]) {
							...
							if (object_data && object_data.data) {
								var model_datas = object_data.data,
									model_data_idx, model_datas_length = model_datas.length, model_data,
								...
								for (model_data_idx = 0; model_data_idx < model_datas_length; ++model_data_idx) {
									...
									model_data = {model: {class_name: model_raw_data.c || object.model_class, data: model_raw_data.d || model_raw_data}};
									if (!model_raw_data.c) {
										if (debug()) {
	...
	MM.prototype.listBindedEvents = function(obj) {
		var _self = this, listeners = obj._listeners,
			events = obj._events,
	...
	MM.prototype._getObjectName = function(obj) {
		var name, type, types = [
			{name : 'collection', class_obj : GameCollections},
			{name : 'model', class_obj : GameModels},
			{name : 'controller', class_obj : GameControllers},
			{name : 'view', class_obj : GameViews}
		];
