/*global Game, GameData, GameEvents */
...
(function () {
	...
	var LGS = {
		...
		renderSpells : function() {
            ...
            this.$content.find('.info_icon').tooltip(tooltip, {width: 400});
		},
		...
		openSpellsMenu: function() {
			...
			if (this.$parent.hasClass('gods_spells_active')) {
                this.$god_containers.slideUp(SPELLS_SLIDE_DURATION, function () {
                    this.$menu.animate({right: MENU_CLOSED_RIGHT}, MENU_SLIDE_DURATION, function () {
                ...
                this.$menu.animate({right: MENU_OPEN_RIGHT}, MENU_SLIDE_DURATION, function () {
		...
		initializeSpellsContainerComponents : function() {
			...
			this.$content.find('.js-power-icon').each(function(index, el) {
				...
				}).on('mouseover', controller.btnSpellMouseOverHandler.bind(controller)), 'powers_buttons');
			});
		...
		initButtonsStates : function() {
			var gods = this.player_gods.getGodsInTowns(),
				god_idx, gods_length = gods.length, god;
