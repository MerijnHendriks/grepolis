/*globals DM, CM, WndHandlerDefault, _, BBCode, WMap, jQuery, Alliance, Layout, reservationTool, GameEvents,
gpAjax, GPWindowMgr, GameData */
...
(function($) {
	...
	WndHandlerAlliance.prototype.onRcvData = function(data, controller, action){
		...
		if (controller === 'alliance') {
			...
			switch (action) {
				...
				case 'invitations':
					$('#generate_new_link').click(function() {
		...
		var registerAllianceFinderComponents = function(index, element) {
			...
			if (recruit_state === 'open') {
				...
				CM.register(that.wnd.getContext(), 'ally_application_button_' + alliance_id, $el.button({
					...
					that.wnd.requestContentGet('alliance', 'create_application', {alliance_id : alliance_id});
				}));
			}
	...
	WndHandlerAlliance.prototype.inviteIntoAlliance = function(){
		...
		this.wnd.requestContentPost('alliance', 'invite', {player_name : player_name});
	};
	...
	WndHandlerAlliance.prototype.massInviteIntoAlliance = function(){
		...
		this.wnd.requestContentPost('alliance', 'massInvite', {alliance_name : alliance_name});
	};
	...
	WndHandlerAlliance.prototype.createAlliance = function(name){
		this.wnd.clearMenu();
		this.updateMap = true;
		this.wnd.requestContentPost('alliance', 'create', {name: name}, function(objGPWnd, data) {
			$.Observer(GameEvents.alliance.join).publish({alliance_id : data.data.alliance_id});
			$.Observer(GameEvents.alliance.create).publish({alliance_name : name});
		});
	};
	...
	WndHandlerAlliance.prototype.leaveAlliance = function(){
		...
		HelperAlliance.leaveAlliance(function() {
			$.Observer(GameEvents.alliance.leave).publish({type_of_leaving : 'leave_alliance'});
	...
	WndHandlerAlliance.prototype.withdrawAllianceApplication = function(application_id){
		this.wnd.requestContentPost('alliance', 'withdraw_application', {id: application_id});
	};

	WndHandlerAlliance.prototype.rejectAllianceInvitation = function(invitation_id){
		this.wnd.requestContentPost('alliance', 'reject_invitation', {id: invitation_id});
	};

	WndHandlerAlliance.prototype.rejectAllianceApplication = function(application_id){
		this.wnd.requestContentPost('alliance', 'reject_application', {id: application_id});
	};
	...
	WndHandlerAlliance.prototype.acceptAllianceApplication = function(application_id){
		this.wnd.requestContentPost('alliance', 'accept_application', {id: application_id});
	};

	WndHandlerAlliance.prototype.rejectMassInvitation = function(invitation_id){
		this.wnd.requestContentPost('alliance', 'reject_mass_invitation', {id: invitation_id});
	};

	WndHandlerAlliance.prototype.acceptMassInvitation = function(invitation_id){
		this.wnd.requestContentPost('alliance', 'accept_mass_invitation', {id: invitation_id});
	};

	WndHandlerAlliance.prototype.membersShow = function(){
		this.wnd.requestContentGet('alliance', 'members_show', {edit: true});
	};

	WndHandlerAlliance.prototype.members = function(){
		this.wnd.requestContentPost('alliance', 'members', {rights: Alliance.fetchMemberRights(),title: Alliance.fetchMemberTitle()});
	};

	WndHandlerAlliance.prototype.pactInvite = function(){
		this.wnd.requestContentPost('alliance', 'pact_invite', {name: $('#ally_pact_invitation_form_name').val()});
	};
	...
	WndHandlerAlliance.prototype.sendApplication = function($alliance_id){
		gpAjax.ajaxPost('alliance', 'get_application_recipients', {alliance_id: $alliance_id}, false, function(data) {
	...
	WndHandlerAlliance.prototype.deleteAlliance = function(){
		...
		Layout.showConfirmDialog(l10n.dissolve_popup_title, l10n.dissolve_application_popup_text, function() {
			...
			self.wnd.requestContentPost('alliance', 'delete', {}, function(_wnd, data) {
				...
				$.Observer(GameEvents.alliance.leave).publish({type_of_leaving : 'remove_alliance'});
	...
	WndHandlerAlliance.prototype.messagePreview = function() {
		var params = {message: $('#application_edit_message').val()};
