...
(function() {
    ...
    function getResourcesForResearches(research, current_academy_level) {
        ...
        return {
            wood : {amount : Math.floor(needed_resources.wood, 10)},
            stone : {amount : Math.floor(needed_resources.stone, 10)},
            iron : {amount : Math.floor(needed_resources.iron, 10)},
            research_points : {amount : research.research_points},
            time: {amount: GameDataResearches.getResearchTime(research, current_academy_level)}
        };
    }
	...
	function getNeededResourcesInfoForAcademy(research, current_academy_level, available_research_points) {
		var result = '', options = {}, itown = ITowns.getTown(Game.townId),
			resources = itown.resources(), current_production = itown.getProduction(),
			time_to_build = 0,
			enough_points = true, enough_resources = true, upgrade_not_possible = options.upgrade_not_possible || false,
