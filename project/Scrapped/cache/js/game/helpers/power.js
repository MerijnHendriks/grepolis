...
(function() {
	...
	var HelperPower = {
		...
		getFuryAmountForAresSacrifice: function () {
			var units = ITowns.getTown(Game.townId).getLandUnits(true),
				max_fury = GameData.powers.ares_sacrifice.meta_defaults.amount,
				result = 0;

			for (var unit_id in units) {
				if (units.hasOwnProperty(unit_id)) {
					if (!GameData.units.hasOwnProperty(unit_id) || result >= max_fury) {
						break;
					}

					result += GameData.units[unit_id].population * units[unit_id];
				}
			}

			return Math.min(result, max_fury);
		}
	};
