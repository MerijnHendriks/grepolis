/*global Layout, _, GPWindowMgr, GameData, alert, $, jQuery, Timestamp, btoa, addslashes, Game */
...
//<%= hCommon.getUnitCountClass(unit_amount) %>
		...
		printTimeInterval : function(timestamp_start, timestamp_stop) {
			...
			var start = new Date(Timestamp.shiftUnixTimestampByTimezoneOffset(timestamp_start, 'player_timezone') * 1E3),
				...
				start_date = fillLeadingZero(start.getUTCDate()), stop_date = fillLeadingZero(stop.getUTCDate()),
			//+1 because months are counted from 0-11
				start_month = fillLeadingZero(start.getUTCMonth() + 1), stop_month = fillLeadingZero(stop.getUTCMonth() + 1);
