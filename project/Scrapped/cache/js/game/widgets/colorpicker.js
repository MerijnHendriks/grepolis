...
(function($) {
	...
	$.fn.colorpicker = function(params) {
		...
		(function() {
			...
			color_selector = $el.find('.color_selector').color_selector({color: '#'+settings.color});
			...
			color_confirm = $el.find('.color_confirm').button({
				...
				icon: true,
				icon_type: 'checkmark',
				tooltips: [{title: settings.l10n.save_color},{}]
			});

			color_default = $el.find('.color_default').button({
				caption: settings.l10n.default_btn,
				tooltips: [{title: settings.l10n.default_color_text},{}]
			});
