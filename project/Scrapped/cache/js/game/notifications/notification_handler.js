/*globals NotificationType, ITowns, Layout, WQM, MilitiaWelcomeWindowFactory, readableUnixTimestamp, MassNotificationType, MM, gpAjax, GameEvents, Game, _, s, Timestamp, GrepoNotificationStack, NotificationFactory */
	...
	function NotificationHandler() {
		...
		function handleDefault(notification) {
			if (notification.type === NotificationType.PHOENICIAN_SALESMAN_ARRIVED) {
				//Check for 'town in notification referecend still is 'ours'
				var town = ITowns.getTown(notification.param_id);
				if (town === undefined) {
					gpAjax.ajaxPost('notify', 'delete', {id: notification.id}, false);
					return;
				}
			}
			...
			_self.notify(
				notification.id,
				notification.type,
				notification.subject + '<span class="small notification_date">' + readableUnixTimestamp(notification.time, 'player_timezone',
					{extended_date: true}) + '</span>',
				notification.time,
		...
		function handleIncomingAttack(notification) {
			...
			if (Layout.player_hint_settings !== undefined) {
				var notification_subject = notification.subject,
					town = {id: (notification.param_str ? $.parseJSON(notification.param_str).town_id : false) || Game.townId},
					current_town_model = MM.getCollections().Town[0].getCurrentTown(),
					town_name = notification.param_str ? $.parseJSON(notification.param_str).town_name :  '',
					incoming_attack_count, show_hint;

				incoming_attack_count = CommandsHelper.getTotalCountOfIncomingAttacks();

                show_hint = !Layout.militia_hint_shown &&
				...
				if (show_hint) {
					...
					Layout.militia_hint_shown = true;
				}

				notification_subject += '<br/><span class="small">'	+ _('Arrival:') + ' ' +
					readableUnixTimestamp(notification.time, 'player_timezone', {extended_date: true});
		...
		function handleIncomingSupport(notification) {
			var town_name = notification.param_str ? $.parseJSON(notification.param_str).town_name :  '';
            var notification_subject = notification.subject +
				'<br/><span class="small">' +
				_('Arrival:') + ' ' +
		...
		function handleBuildingFinishedNotification(notification) {
			...
			//notification.game_initialization
			notification.subject = getSubject($.parseJSON(notification.param_str));
		...
		function handleAllRecruitmentFinishedNotification(notification) {
			...
			notification.subject = getSubject(notification.subject, notification.param_id, $.parseJSON(notification.param_str));
		...
		function handleNewMessageNotification(notification) {
			...
			var params = $.parseJSON(notification.param_str);
			if (params) {
		...
		function handleMassNotification(notification) {
			var str_params = $.parseJSON(notification.param_str),
		...
		this.notify = function (id, type, title, time, param_id, param_str, game_initialization) {
			var interval = 1E4, ttl, dupe = false;
