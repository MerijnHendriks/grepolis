...
(function() {
	...
	var InventoryItem = function() {}; // never use this, becasue it will be overwritten
	...
	InventoryItem.trash = function() {
		this.execute('trash', {inventory_item_id: this.id}, {
				success: function(data) {
	...
	InventoryItem.getTooltipData = function() {
		...
		if (type === 'power_reward_effect_inventory_item') {
			...
			power = us.find(GameData.powers, function(item) { return (item.id === that.getProperties().power_id); });
			...
			power = us.find(GameData.powers, function(item) { return (item.name === GameData.inventoryItems[type].name); });
			tooltip_data = {
