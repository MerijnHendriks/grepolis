...
window.WndHandlerAttack = (function() {
	...
	/**
	 * select all units from current town
	 *
	 */
	WndHandlerAttack.prototype.selectAllUnits = function() {
		//var inputs = .find('input.unit_input');
		var units = ITowns.getTown(Game.townId).units(),
		self = this;

		this.getUnitInputs().each(function() {
			var newVal = self.validateMaxPerAttack(this.name, units[this.name], self.wnd.getJQElement().find('form').data().type);
	...
	WndHandlerAttack.prototype.sendUnits = function(sending_type, popup_type, target_id, confirmed) {
		...
		if(GameDataHeroes.areHeroesEnabled()) {
			...
			if (cbx_include_hero) {
				...
				if (cbx_include_hero.isChecked()) {
					var hero_id = this.getHeroInTheTown().getId();

					params.heroes = hero_id;
		...
		this.wnd.ajaxRequestPost(popup_type, 'send_units', params, {
			success: function(window, data) {
				$.Observer(GameEvents.command.send_unit).publish({sending_type:params.type, target_id:target_id, params: params}); //TODO - name it properly
	...
	WndHandlerAttack.prototype.render = function(action) {
		var root, data = this.data, _self = this;
		...
		if (action === 'attack' || action === 'support') {
			var $btn_plan_attack = root.find('#btn_plan_attack_town'), has_captain = $btn_plan_attack.attr('data-hascaptain') === '1';
			...
			this.$btn_runtime = root.find('#btn_runtime').button({
				...
				FactoryRuntime.openWindow(data.target_id, {is_portal_command: that.is_portal_command});
			});
	...
	WndHandlerAttack.prototype.initializeHeroCheckbox = function() {
		var _self = this, root = this.wnd.getJQElement(), context = this.wnd.getContext(),
		...
		if (is_hero_in_town || is_hero_being_assigned_to_town) {
			if (is_hero_in_town) {
				...
				if (hero_in_town.isInjured()) {
					...
					$portrait.tooltip(TooltipFactory.getHeroCard(hero_in_town.getId(), {hero_level: hero_in_town.getLevel()}), {}, false);
				}
			}
    ...
    WndHandlerAttack.prototype.getAttackTableBoxHeader = function(type) {
		...
		return us.template(template, {title: l10n.headers[type]});
	};
	...
	WndHandlerAttack.prototype.registerViewComponents = function() {
		...
		if (this.action === 'support') {
			...
			CM.register(context, 'btn_open_support_window', root.find('.btn_open_support_window').button({
				disabled : us.isEmpty(supporting_units), tooltips : [
					{title : l10n.show_troops, hide_when_disabled : true}
				]
			}).on('btn:click', function() {
	...
	WndHandlerAttack.prototype.onRcvData = function(data, controller, action, obj) {
		...
		if (data.tmpl) {
			GameData.add({'TownAttackTemplate' : data.tmpl});
		}
	...
	WndHandlerAttack.prototype.bindDurationCounter = function() {
		...
		function recalcDuration() {
			...
			if (duration !== null) {
				...
				// get root element
				var prefix = that.is_farm_town || that.support ? '' : '~';
