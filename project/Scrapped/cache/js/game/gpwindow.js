/* global GameControllers, GPAjax, CM, WM, Game, Layout, GPWindowMgr, GameEvents */
(function() {
	'use strict';

	function GPWindow(id, wndmgr, _models, _collections) {
		var wndid = id, wndname = 'gpwnd_' + id,
			rootnode,
			ajaxloader,
			loader,
			jqrootnode,
		...
		var cm_context = {main : null, sub : null};
		...
		this.createWindow = function (title, UIopts, additional_data) {
			...
			if (UIopts.minimizable && !is_not_closable) {
				minimizeElem = $('<a>').attr({'href' : '#', 'class': 'ui-dialog-titlebar-minimize ui-corner-all'}).click(function() {
			...
			if (UIopts.help) {
				help = $('<a>').attr({'href' : '#', 'class': 'ui-dialog-titlebar-help ui-corner-all'}).click(this.showHelp);
			...
			if (UIopts.yOverflowHidden) {
				jqcontent.css({'overflow-y': 'hidden'});
			}
		...
		this.getPosition = function () {
			var rect = {position: jqrootnode.dialog('option', 'position'),
		...
		this.initMenu = function (data, controller, action) {
			...
			function arrangeScroll() {
				var button_sum, titlebar, tab_width, header_width;
				// scrolling
				...
				// not working because buttons are not yet appended to the document ...
				button_sum = 28 + 21 + 21;
			...
			// needed for IE7, otherwise sometimes words break at a nbsp while the
			// width is computed
			...
			var i, tmp,
				...
				clickHandler = function fooooooo() {
					...
					if (menu[i].id) {
						...
						obj = menu[i].type ? {'type': menu[i].type} : menu[i].obj;
			...
			// clone array
			var foo = naturalWidth.map(function (i) {return i[0];}, this),
		...
		this.requestContent = function (controller, action, params, callback, external_reload, method) {
			...
			var _self = this, obj, http_status_code_options;
			...
			obj = {
				success: function (_context, _data, _flag, _t_token) {
					...
					if(wndhandler){
						if (action === 'default') {
							var menu_tabs = $.parseJSON(_data.menu || menuData);
						...
						_self.setContext({main : _self.getID(), sub : curr_controller + '_' + curr_action});
			...
			if (method === 'get') {
				http_status_code_options = {close_wnd: true, wnd: this};
		...
		this.setFocus = function(value) {
			var will_be_focused = value, is_currently_focused = focused;
		...
		this.minimizeWindow = function() {
			if (this.isMinimizable()) {
				var html  = minElm.html = $('<div id="' + jqcontent[0].id +'_min" class="gpwindow_minimize left"><div class="right"><div class="center"></div></div></div>'),
					close = minElm.close = $('<a>').attr({'href':'#', 'class':'ui-dialog-titlebar-close ui-corner-all'}).click(that.close),
					max   = minElm.max = $('<a>').attr({'href':'#', 'class':'ui-dialog-titlebar-maximize ui-corner-all'}).click(that.maximizeWindow),
					title = minElm.title = $('<span>').attr({'href': '#', 'class': 'ui-dialog-title'}).css({'cursor':'pointer'}).html(options.title).click(that.maximizeWindow),
