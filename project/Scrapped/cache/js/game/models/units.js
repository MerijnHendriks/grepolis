...
(function() {
	...
	function reduceUnitType(closure, start, context) {
		var unit_type_idx, unit_types_length = Units.UNIT_TYPES.length, unit_type,
			result = start;
	...
	Units.getHeroes = function() {
		var player_hero, player_heroes = [], heroes = this.get('heroes'), i, hl;
	...
	Units.calculateRemainingUnitsAfterSubstraction = function(units_to_substract) {
		var units = this.getUnits(), unit_id;
	...
	Units.getRuntimes = function(callback) {
		this.execute('getRuntimes', {id: this.id}, callback);
	};
