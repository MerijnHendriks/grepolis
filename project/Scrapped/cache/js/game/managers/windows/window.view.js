...
(function(window_manager_views) {
	...
	window_manager_views.WindowView = Backbone.View.extend({
		...
		changePosition : function(ver, hor) {
			var window_model = this.controller.getModel('window'), $window = this.$window,
				props = {position : 'absolute'}, resizable = window_model.isResizable(),
				position = window_model.getPosition();

			var viewport_size = {
				width : $('body').innerWidth(), //this.$curtain.outerWidth(),
				height : this.$curtain.outerHeight()
			};
