...
CommandsOverview, TradeOverview, CultureOverview, GodsOverview, OuterUnitsOverview, HidesOverview */
...
(function() {
	...
	WndHandlerTownOverviews.prototype.onInit = function(title, UIopts, town_id) {
		...
		this.wnd.requestContentGet(UIopts.controller, UIopts.overview_type, {'town_id' : town_id});
...
//	this.wnd.requestContent('town_info', arguments[0], {id: arguments[1]});
	...
	WndHandlerTownOverviews.prototype.initializeResourcesCounter = function(towns, hides) {
		...
		var step = 500;
		var town, town_id;
		var root = this.wnd.getJQElement(), html_spinner, limit, hide_limit;
		...
		for (town_id in towns) {
			...
			if (hides) {
				...
				if (html_spinner) {
					HidesOverview.spinners[town.id] = html_spinner.spinner({value : 0, step : 500, max : limit});
				}
			}
		}
	...
	WndHandlerTownOverviews.prototype.handleUnitUpdate = function(event) {
		if (event.type === GameEvents.town.units.change) {
			var units = ITowns.getTown(Game.townId).units(), unit_id;
