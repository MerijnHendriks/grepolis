...
window.PremiumWindowFactory = (function() {
	...
	return {
		...
		openBuyAdvisorsWindow: function() {
			var wnd = hCommon.openWindow(GPWindowMgr.TYPE_PREMIUM, null, {noInitRequest : true}, 'premium_features', 'index', {}, 'get', function() {});
		...
		_openCashShopWidnow : function(shop_url) {
			var CashShopWindowFactory = require('features/cash_shop/factories/cash_shop');
			CashShopWindowFactory.openWindow(shop_url);
		},
		...
		_openPremiumWindow : function(category, link) {
			...
			if (!window) {
				window = GPWindowMgr.Create(GPWindowMgr.TYPE_PREMIUM, l10n_premium.premium, {sub_content : 'premium_overview', callback : callback});
			} else {
