...
(function($) {
	...
	model = {
		...
		removeTarget : function(town_id) {
			var targets = data.target_list, l = targets.length;
		...
		getTarget : function(town_id) {
			var targets = data.target_list, i, l = targets.length;
		...
		deleteAttack : function(attack_id) {
			var attack, attacks = data.origin_town_list, l = attacks.length;
		...
		getAttack : function(attack_id) {
			var attack, attacks = data.origin_town_list, i, l = attacks.length;
	...
	view = {
		...
		initializeMainLayout : function() {
			...
			CM.register(cm_context, 'btn_edit_rights', $content.find('.btn_edit_rights').button({
				caption : l10n.edit_rights, template : 'empty'
			...
			CM.register(cm_context, 'rbtn_sort_attack_list', $content.find(".rbtn_sort_attack_list").toggleStateRadiobutton({
				value : 'arrival_at', template : 'tpl_rb_sort_by', options : [
					{value : 'town_name', tooltip : l10n.order_by_town_name},
					{value : 'send_at', tooltip : l10n.order_by_send_at},
			...
			CM.register(cm_context, 'btn_remove_target', $content.find('.btn_remove_target').button({
				template : 'empty', tooltips : [{title : l10n.remove_target}]
			}).on('btn:click', function() {
				...
				}, l10n[is_shared ? 'delete_all' : 'delete_item'], function() {
				}, l10n.cancel).setHeight(220);
			}));

			//Attacks buttons
			CM.register(cm_context, 'btn_add_attack', $content.find('.btn_add_attack').button({
				template : 'empty', tooltips : [{title : l10n.add_attack}]
			}).on('btn:click', function() {
			...
			CM.register(cm_context, 'btn_remove_attack', $content.find('.btn_remove_attack').button({
				template : 'empty', tooltips : [{title : l10n.remove_attack}]
			}).on('btn:click', function() {
			...
			CM.register(cm_context, 'btn_go_to_plan_list', $content.find('.btn_go_to_plan_list').button({
				caption : l10n.go_to_plan_list, tooltips : [{title : l10n.go_to_plan_list}], template : 'internal'
			}).on('btn:click', function() {
		...
		initializeTargetsList : function() {
			...
			$targets_list.off('click.target_list').on('click.target_list', '.target_row', function(e) {
				var $row = $(e.currentTarget), $target = $(e.target),
	...
	controller = {
		initialize : function(obj) {
			...
			cm_search_by = {main : cm_context.main, sub : 'search_by'};
