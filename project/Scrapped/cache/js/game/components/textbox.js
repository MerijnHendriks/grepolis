...
(function($, w) {
	...
	$.fn.textbox = function(params) {
		...
		function setSelection(start, end) {
			var sel = typeof start === 'number' && typeof end === 'number' ? {start : start, end : end} : settings.selection_details;
		...
		function checkConditions(value) {
			var max = settings.max, min = settings.min, type = settings.type,
				correct_mistakes = !settings.invalidmsg,
				new_val = value, error = false;
		...
		function setValue(value, props) {
			...
			var new_val = checkConditions(value), old_val = settings.value;
