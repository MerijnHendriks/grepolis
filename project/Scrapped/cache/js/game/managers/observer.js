...
(function($, GameEvents) {
	...
	function displayObjectProperties(data) {
		var obj = data.obj,
			...
			objProp, length = 0, /*subGroupLength = 0,*/
			events = $._data(document, 'events'), i, l, listeners, count = 0;
	...
	$document.on('keydown.observer', function(e) {
		var key = e.keyCode, is_input = !!(e.target && e.target.tagName.match(/INPUT|TEXTAREA|SELECT|OPTION/));
