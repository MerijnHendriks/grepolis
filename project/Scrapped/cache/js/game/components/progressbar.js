	...
	$.fn.singleProgressbar = function(params) {
		...
		function updateProgress(value) {
			var max = settings.max,
				...
				step_size = settings.max / settings.step_count, animation_step = 0,
				$animate = $indicator.find('.js-animate'),
				total_value,
				factor, animation_settings = {},
				progress_size,
				real_max = settings.real_max, real_value;
			...
			if (type === 'time') {
				...
				if (value > max) {
					...
					$indicator.attr('class', 'indicator s_' + settings.step_count).delay(500).queue(function (next) {$indicator.attr('class', 'indicator s_0');next();});
				}
		...
		this.updateTooltipData = function(state, data) {
			var tooltip_data = settings.tooltips[state].data, data_id;
		...
		this.setMax = function(value, props) {
			...
			if (typeof circular_progress !== 'undefined') {
				circular_progress.setMax(value, {silent : silent});
			}
