...
(function(window) {
	...
	var GeneralHelper = {
		...
		getCastedPowersIds : function() {
			var ids = [], casted_powers = this.getCastedPowers(), i, l = casted_powers.length;
	...
	var GeneralModifications = {
		...
		getStorageVolumeModification : function() {
			var storage_volume_modifier = 1.0,
				player = MM.getModelByNameAndPlayerId('Player');

			if (player.getDominationArtifactUnlocked()) {
				storage_volume_modifier += GameData.artifacts.storage_modification_for_domination_artifact;
			}

			return storage_volume_modifier;
		},

		getStorageVolumeBonus : function() {
			// you can't use premium features in this function!
			var storage_volume_bonus = 0,
				researches = ITowns.getTown(Game.townId).researches(),
				finished_wonders = us.last(MM.getCollections().Wonder);

			// Forschung: Keramik
			if (researches.hasResearch('pottery')) {
				storage_volume_bonus += GameDataResearches.getBonusPotteryStorage();
			}

			// Weltwunder: Pyramide
			if (finished_wonders.hasWonder('great_pyramid_of_giza')) {
