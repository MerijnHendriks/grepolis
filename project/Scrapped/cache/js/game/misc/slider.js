...
(function() {
	...
	function Slider(options) {
		...
		try {
			this._elementSlider.slider({'animate': true, 'orientation' : this._orientation , 'min' : this._min , 'max' : this._max , 'value' : this._value , 'step' : this._step});
		} catch(e) {}
		...
		if (this._elementMin != null && this._elementMax != null) {
			this._elementMin.click((
				function() {this.setValue(this.getMin());}
			).bind(this));
			this._elementMax.click((
				function() {this.setValue(this.getMax());}
			).bind(this));
		}
		...
		this._elementInput.bind('change', {self: this}, function(e) { //pass self reference as event data
			e.data.self.setValue(this.value);
		});

		this._elementInput.focus(function () {this.select();}); // TODO: Test IE compatibility
	}
	...
	Slider.prototype.setMin = function(min) {
		this._min = min;
		if( this._elementMin ) {this._elementMin.text(min);}
