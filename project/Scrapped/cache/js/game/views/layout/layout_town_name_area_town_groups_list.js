	...
	var LayoutTownNameAreaTownGroupsList = BaseView.extend({
			...
			if (this.controller.isTownGroupsDragDropEnabled()) {
				$('#town_groups_list .item').draggable({
					...
					helper: function () {
						...
						return $el.clone().css({width : $el.width()});
					},
					scope: 'town_group'
				});
				...
				$('#town_groups_list .group_towns').droppable({
					drop : function(event, ui) {
						var $group = $(this), $item = ui.draggable,
		...
		*/
		organizeMenuInColumns : function() {
			var $list = this.$list, $group, $inner_column;
			var group, analized_data = this.controller.getTownGroupsAnalizedData(), i, l = analized_data.length,
				row_height = this.ROW_HEIGHT, row_width = this.ROW_WIDTH;
		...
		calculateListSize : function(analized_data) {
			var i, l = analized_data.length, group,
				rows_in_column = 0, nr_of_columns = 0;
		...
		calculateListPosition : function() {
			var $list = this.$list, $cont = this.$parent.find('.town_groups_dropdown'),
				width = $cont.outerWidth(true), height = $cont.outerHeight(true),
				offset = $cont.offset();
