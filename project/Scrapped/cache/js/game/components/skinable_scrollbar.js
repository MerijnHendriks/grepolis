...
(function($) {
	...
	$.fn.skinableScrollbar = function(params) {
		...
		var _self = this, $el = $(this), $document = $(document), isiOS = Game.isiOs(),
		...
		function prepareHtmlElements() {
			...
			$elements_to_scroll.css({position : settings.elements_to_scroll_position});
		...
		function scrollElements(value) {
			var slider_position = Math.round(getValidSliderPosition(value));
			var scroll = max_scroll ? Math.min(0, Math.max(-max_scroll, -(max_scroll / max_position) * slider_position)) : 0;
			...
			//localy_disabled = $arrow1.hasClass('disabled') && $arrow2.hasClass('disabled');
		}
		...
		function bindEvents() {
			...
			$slider_container.on('click.' + className, function(e) {
				...
				scrollElements(offset_cursor - offset_slider);
			});
			...
			$arrow1.on('click.' + className, function() {
				...
				var pos_start = _self.getSliderPosition();

				scrollElements(pos_start - 5);
			});
			...
			if (is_vertical) {
				$el.on('mousewheel.' + className, function(e, delta) {
					...
					var pos_start = $slider.position()[pos_method];
			...
			$el.on('touchstart.' + className, function(e) {
				...
				var scroll_pos = $slider.position()[pos_method],
					client_start_pos = e[touch_client_method],
					client_move_pos;
				...
				$document.on('touchmove.' + className, function(e) {
					...
					scrollElements(scroll_pos + client_start_pos - client_move_pos);
