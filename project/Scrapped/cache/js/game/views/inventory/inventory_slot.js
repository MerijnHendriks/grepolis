...
(function() {
	...
	var InventorySlotView = BaseView.extend({
		...
		registerComponents : function() {
			...
			if (this.model.getId() === 0) {
				...
				this.registerComponent('btn_unlock_' + this.index, this.$el.find('.button_new').button({
					...
					icon_position: 'right',
					tooltips: [
						{title: '<h4>' + _('Add a new slot') + '</h4>' + s(_('You can add a new inventory slot for %1 gold.'), GameDataInventory.getSlotCost(this.index + 1)), styles : {width :400}, hide_when_disabled : true}
					]
		...
		_getSlotClassName : function() {
			var type = this.model.getType(), class_name = '', is_empty = type === 'empty';
