...
(function() {
	...
	var SupportOverviewMainView = BaseView.extend({
		...
		render : function() {
			var controller = this.controller, $el = this.$el, l10n = this.l10n;
		...
		initializeComponents : function() {
			var $el = this.$el, controller = this.controller;
			...
			if (controller.isSortOptionVisible()) {
				...
				controller.registerComponent('dd_support_sort_by', $el.find('#dd_support_sort_by').dropdown({
					value : controller.getDefaultSortOption(), options : controller.getSortDropdownOptions()
