/* global MM, Quickbar, GameData, PlayerInfo, us, gpAjax, HumanMessage, BuyForGoldWindowFactory, url, Game, WF, GPWindowMgr */
(function() {
	'use strict';
	...
	WndHandlerPlayerSettings.prototype.onRcvData = function(data, controller, action) {
		...
		if (this.wnd.getJQElement().find('.settings-menu').length === 0) {
			if (data.backbone && !GameData.settingsBackboneTemplate) {
				GameData.add({settingsBackboneTemplate : data.backbone});
				GameData.add({settingsBackboneLinks : data.links});
			}

			this.wnd.setContent2(us.template(data.backbone || GameData.settingsBackboneTemplate, {menu : $.parseJSON(data.links || GameData.settingsBackboneLinks)}));

			//Click handler for the settings menu
			this.wnd.getJQElement().find('.settings-menu').unbind('click').click(function(e) {
				var target = $(e.target), id, controller = that.controller, action = that.action;
		...
		form.unbind().bind('keydown', function(e) {
...
// TODO: check if it can be easily removed and remove that
	WndHandlerPlayerSettings.prototype.saveSettings = function(reload_grepo) {
		...
		var setting_types = [
			...
			'webnotification_combat_hero_healed',
...
//shared internet connections
	...
	WndHandlerPlayerSettings.prototype.deleteSharedConnection = function(shared_player_id) {
		...
		this.wnd.ajaxRequestPost('player', 'delete_shared_connection', {shared_player_id: shared_player_id}, function(window, data) {
			//insert html
...
//buy vacation days
	WndHandlerPlayerSettings.prototype.buyVacationDays = function (buy_vacation_days_gold, button) {
		var days = parseInt($('#vacation_order_input').val(), 10), disabled = 'inactive', confirm, that = this;
		...
		button.disable = function() { button.addClass('disabled');};
		button.enable = function() { button.removeClass('disabled');};

		confirm = function() {
			gpAjax.ajaxPost('player', 'buy_vacation_days', {days: days}, false, function (data) {
	...
	WndHandlerPlayerSettings.prototype.sendDeleteAccountRequest = function() {
		...
		gpAjax.ajaxPost('player', 'do_delete_account', {password: password});
	};
	...
	WndHandlerPlayerSettings.prototype.sendRestartMailRequest = function() {
		...
		gpAjax.ajaxPost('player', 'request_restart_email', {confirmed: confirmed, password: password}, true);
	};
	...
	WndHandlerPlayerSettings.prototype.openSurveyResults = function() {
		var windows = require('game/windows/ids');

		WF.open(windows.SURVEY);
	};
