		...
		$(document.body).on('touchstart', 'a:not(.fixediOsLink)', function(e){
			...
			if ($this.is('[class^="gp_"]')){
				...
				if(tmp){
					...
					$this.on('click', function(event){ event.preventDefault(); }).addClass('fixediOsLink').attr('href', '#');
				}
			} else {
				$this.on('click', function(event){ event.preventDefault(); }).addClass('fixediOsLink');
			}
		});
	}

	window.setupLinkHandleriOs = setupLinkHandleriOs;
}());
