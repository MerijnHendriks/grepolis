...
window.BuildingWindowFactory = (function() {
	...
	return {
		open : function(building_type, action, params) {
			var w = this.getWnd(), type, title, handler, options = {};

			$.Observer(GameEvents.window.building.open).publish({building_id : building_type});
			...
			switch (building_type) {
				...
				case Buildings.STORAGE:
                    ...
                    return WF.open(building_type, {collections: {towns: MM.getCollections().Town[0]}});
		...
		refreshAllWindows : function() {
			...
			var all_opened_windows = GPWindowMgr.getAllOpen(), i, l = all_opened_windows.length, wnd, handler;
