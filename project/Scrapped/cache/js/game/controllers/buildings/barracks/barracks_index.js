	...
	var BarracksController = GameControllers.TabController.extend({
		...
		getUnits : function() {
			var building_type = this.getBuildingType(),
				itown = ITowns.getTown(Game.townId), iunits = itown.units(),
				support_units = itown.unitsSupport(), outer_units = itown.unitsOuter(),
				unit_id, gd_unit, gd_units = GameData.units, god_id = itown.god(),
				returned_units = [], unit_cost,
