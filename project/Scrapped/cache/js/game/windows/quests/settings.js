...
(function(views, collections, models, settings) {
	...
	settings[window_type] = function(options) {
		return us.extend({
			...
			tabs : [
				{type : tabs.INFO, title : '', content_view_constructor : views.QuestInfo, hidden : true}
			],
			width : 669,
			max_instances : 1,
			minimizable : false
	...
	settings[window_type_progress] = function(options) {
		return us.extend({
			...
			tabs : [
				{type : tabs.PROGRESS, title : '', content_view_constructor : views.QuestProgress, hidden : true}
			],
			width : 669,
			max_instances : 1,
			closable: false,
	...
	settings[window_type_welcome] = function(options) {
		return us.extend({
			...
			tabs : [
				{type : tabs.WELCOME, title : '', content_view_constructor : views.QuestWelcome, hidden : true}
			],
			width : 850,
			max_instances : 1,
			minimizable : false,
