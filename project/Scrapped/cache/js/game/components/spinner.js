...
(function($) {
	...
	$.fn.spinner = function(params) {
		var _self = this, settings, type = (params || {}).type || 'integer';
		...
		function bindEvents() {
			...
			$el.on('updated:max', function(/*e, new_val, old_val*/) {
				_self.setValue(settings.value, {silent : true});
			});
			...
			$input.on('blur.spinner', function(/*e*/) {
				...
				_self.setValue($input.val(), {force : true});
			});
		}
		...
		function checkConditions(val) {
			var type = settings.type,
				max = settings.max, min = settings.min;
