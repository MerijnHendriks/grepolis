/* global Timestamp, GameDataPowers, DeltaProperty, GrepolisModel */
...
(function() {
	...
	PlayerGods.onGodsFavorFull = function(obj, callback) {
		...
		self.onGodsFavorChange(obj, function() {
			var full_gods = GameDataGods.getAllGods()
			.map(function(god_id) {
				return {
					id: god_id,
					favor: self.get(god_id + '_favor')
				};
			})
			.filter(function(god){
				return god.favor >= self.getMaxFavor();
			})
			.map(function(god) {
				return god.id;
			});
