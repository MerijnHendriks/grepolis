...
window.Alliance = (function() {
	...
	var Alliance = {
		...
		save: function(target, callback) {
			...
			var params = {field: target, value: textarea.val()};
		...
		doKickPlayer: function(player_id) {
			Alliance.action('kick', {'player_id': player_id}, function(data) {
		...
		cancel_invitation: function(id) {
			Alliance.action('cancel_invitation', {id: id}, function(data) {
				$('#invitation_' + id).fadeOut();
				// we do not have the context of the Window, so we fake a reload to fix the UI
				$('a#alliance-invitations').click();
			}.bind(id)
						   );
		},

		cancel_mass_invitation: function(id) {
			Alliance.action('cancel_mass_invitation', {id: id}, function(data) {
		...
		inviteFriends: function() {
			...
			Game.invitation_path = {src: 'alliance'};
