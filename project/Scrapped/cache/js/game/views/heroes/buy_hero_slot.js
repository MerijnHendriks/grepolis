...
(function() {
	...
	var HeroesBuyHeroSlot = BaseView.extend({
		...
		registerViewComponents : function() {
			var _self = this, controller = this.controller, l10n = this.l10n, currency_id = 'gold',
				premium_slot_cost = GameDataHeroes.getSlotCost(), player_ledger = this.player_ledger;
			...
			this.$el.find('.btn_buy_premium_slot').each(function(index, el) {
				...
				controller.registerComponent('btn_buy_premium_slot' + index, $btn.button({
					caption : cost, icon : true, icon_type : currency_id,
					disabled : false,
					tooltips : [
