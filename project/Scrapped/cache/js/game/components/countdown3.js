...
(function() {
	...
	function convertSeconds(value, display, only_non_zero) {
		...
		switch (display) {
			...
			case 'day_hr_min':
				return day_hr_min_sec(sec_to_display, only_non_zero, {with_seconds: false});
	...
	CD.prototype.setValue = function(value) {
		if (value >= 0) {
			...
			TM.register(this.uid, interval, this.tick.bind(this.$el), {timestamp_end: this.timestamp_end});
		}
	};
