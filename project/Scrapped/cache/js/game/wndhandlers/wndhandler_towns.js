(function() {
	...
	WndHandlerTowns.prototype.updateTradeCapValue = function() {
		if($('#avlbl_cap').length){
			...
			], sum = parseInt($('#avlbl_cap').val(), 10), i = values.length;
	...
	WndHandlerTowns.prototype.castPower = function(power_id, town_id, castedFromTownView, castedFromGodsOverview, callback) {
		var _self = this, gd_power = GameData.powers[power_id],
		...
		function castPower(context, power_id, town_id) {
			...
			gpAjax.ajaxPost('town_info', 'cast', {'power' : power_id, 'id' : town_id}, false, function (data) {
				...
				if (typeof data.finished_at == 'number') {
					$.Observer(GameEvents.command.cast_power).publish({power_id: power_id, town_id: town_id, data: data});
				} else {
					$.Observer(GameEvents.command.cast_power).publish({power_id: power_id, town_id: town_id});
				}
