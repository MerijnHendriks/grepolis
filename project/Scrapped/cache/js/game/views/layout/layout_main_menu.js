	...
	var LayoutMainMenu = BaseView.extend({
		...
		setButtonDefinition : function() {
			...
			if (Features.isDominationActive()) {
				this.buttons_definition.splice(4, 0, {id : 'domination'});
			} else if (Features.isOlympusEndgameActive()) {
				this.buttons_definition.splice(4, 0, {id : 'olympus'});
			}
		},
		...
		render: function() {
			...
			this.menu_height = this.$content_menu.height();
			this.$content_menu.addClass('no-transitions').height(this.menu_height).removeClass('no-transitions');
		...
		slideToggle: function() {
			if (this.visible) {
				this.$content_menu.find('.content_wrapper .button_wrapper, .content_wrapper .name_wrapper').animate({opacity: 0}, {
					duration: 200
				});
				this.$content_menu.delay(100).slideUp({duration: 300});
				this.$el.addClass('container_hidden');

				this.visible = false;
			} else {
				this.visible = true;
				this.$content_menu.slideDown({duration: 300});
				this.$el.removeClass('container_hidden');
				this.$content_menu.find('.content_wrapper .button_wrapper, .content_wrapper .name_wrapper').animate({opacity: 1}, {
					duration: 400
				});
			}
		},
