	...
	var GameDataPowers = {
		...
		getCssPowerId : function(casted_power) {
			...
			var gd_power = GameData.powers[casted_power.power_id],
				configuration = $.extend({casted_power_end_at : casted_power.end_at}, casted_power.configuration),
		...
		getCssPowerIdWithLevel : function(css_power_id, level, skin, power_id) {
			if (GameData.powers[power_id].passive) {
				level = 0; // Passive powers will not show level icons
			}

			return css_power_id + (level ? ' lvl lvl' + level : '') + (skin ? ' ' + skin : '') +
		...
		getRewardCssClassIdWithLevel : function(reward_arg) {
			...
			return GameDataPowers.getCssPowerIdWithLevel(tooltip_data.i_id, level, skin, gd_power.id);
		},
		...
		getTooltipPowerData: function (gd_power, configuration_arg, level, progress) {
			var configuration = $.extend({}, configuration_arg),
				...
				meta_idx, meta_fields_length = gd_power.meta_fields && gd_power.meta_fields.length,
				meta_field, config_value, power_name = gd_power.name,
                needed_population = {},
				skin = '',
				current_progress = 0,
				limit = 0,
			...
			var replacement_data = [], replaceBy = function(match, pos) {
				//Remove the "%" from "%x"
			...
			for (meta_idx = 0; meta_idx < meta_fields_length; ++meta_idx) {
				...
				if ((gd_power.id === 'unit_training_boost' || gd_power.id === 'instant_unit_package') && meta_field === 'type_plural' && configuration.type) {
				...
				if ((meta_field === 'limit' || meta_field === 'progress') && battlepoint_power_ids.includes(gd_power.id)) {
					if (typeof progress !== 'undefined' && progress !== null && progress !== false) {
						...
						config_value = configuration[meta_field].battlepoints;
					}
				}

				if ((gd_power.id === 'unit_training_boost' || gd_power.id === 'instant_unit_package') && (meta_field === 'type' || meta_field === 'type_plural')) {
