/* global DeprecatedHelper, MM, HumanMessage, ConfirmationWindowFactory, BuildingPlace, TooltipFactory,  WMap, TownRelationProvider, Slider, ITowns, us, GameData, WndHandlerBuilding, Game, GameEvents, Timestamp, GPWindowMgr, CM, DM, Backbone */
...
(function() {
	...
	WndHandlerFarmTown.prototype.onInit = function(title, UIopts, id) {
		...
		this.wnd.requestContentGet('farm_town_info', UIopts.action, {id: id}, function() {
			var i;
			var sim_units = {att: {}, def: {}};
			...
			$('#farm_town_simulate, #farm_town_simulate_tooltip').bind('click', function() {
				_self.root.find('.farm_attack_troops_troops').find('input[type="text"]').each(function() {
					var amount = $(this).val();
					sim_units.att[$(this).attr('name')] = amount.length ? parseInt(amount, 10) : 0;
				});

				_self.root.find('.farm_attack_farm_troops .unit').each(function() {
					var data = $(this).data();
					if (data.unitId === 'wall') {
						sim_units.def_mods = {building_wall: data.level};
					} else {
	...
	WndHandlerFarmTown.prototype.onRcvData = function(data, controller, action) {
		...
		switch (action) {
			case 'attack':
				...
				if (data.tmpl) {
					GameData.add({'FarmAttackTemplate' : data.tmpl});
				}
			...
			case 'info':
				...
				root.find('.farm_build_res .unit_container.trade_resource span.clickable').click(function() {
				...
				this.spinners.sp_wood = root.find('#trade_type_wood').spinner({
					value : 0, step : 500, max : resources.wood, tabindex : 61
				});
				this.spinners.sp_stone = root.find('#trade_type_stone').spinner({
					value : 0, step : 500, max : resources.stone, tabindex : 62
				});
				this.spinners.sp_iron = root.find('#trade_type_iron').spinner({
					value : 0, step : 500, max : resources.iron, tabindex : 63
				});
				break;
			default:
				break;
	...
	WndHandlerFarmTown.prototype.tradeWithFarmTown = function(target_id, event) {
		...
		this.wnd.ajaxRequestPost('farm_town_info', 'trade', {'id' : target_id, 'trade_input' : trade_input}, {
	...
	*/
	...
	WndHandlerFarmTown.prototype._claimLoadSuccesCallback = function(target_id, claim_type, time) {
		this.wnd.ajaxRequestPost('farm_town_info', 'claim_load', {target_id: target_id, claim_type: claim_type, time: time}, function(window, data) {
			window.close();
	...
	WndHandlerFarmTown.prototype.claimUnits = function(target_id, _unit_type) {
		this.wnd.ajaxRequestPost('farm_town_info', 'claim_units', {target_id: target_id, unit_type: _unit_type}, function(window, data) {
			window.close();
			var lootable_human = DateHelper.readableSeconds(data.lootable_at - Timestamp.now('s'));
			...
			WMap.removeFarmTownLootCooldownIconAndRefreshLootTimers(target_id);
			$.Observer(GameEvents.window.farm.claim_unit).publish({target_id: target_id, unit_type: _unit_type, data:data});
		});
	};
