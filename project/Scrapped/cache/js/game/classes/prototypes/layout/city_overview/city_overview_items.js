...
(function() {
	...
	CityOverviewItems.prototype.getGhostBuildings = function() {
		...
		us.each(buildings, function(building_level, building_id) {
			var requirements_fullfilled = GameDataBuildings.isBuildingRequirementsFullfilled(building_orders, current_town, building_id);
			...
			if (special_building_spots.special1.indexOf(building_id) > -1 && special1_occupied) {return;}
			if (special_building_spots.special2.indexOf(building_id) > -1 && special2_occupied) {return;}
	...
	CityOverviewItems.prototype.areSpecialTutorialQuestsRulesApplied = function(building_id, buildings) {
		 ...
		 * Rules for quests:
		 */
		if (building_id === 'docks' && buildings.main >= 12) {return true;}

		if (building_id === 'academy' && buildings.barracks >= 5) {return true;}

		if (building_id === 'hide' && buildings.market >= 3) {return true;}

		if (building_id === 'tower' && buildings.main >= 12) {return true;}

		if (building_id === 'theater' && buildings.main >= 12) {return true;}

		if (building_id === 'wall' && buildings.main >= 3) {return true;}
