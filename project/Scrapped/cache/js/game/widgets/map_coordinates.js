	...
	$.fn.mapCoordinates = function(params) {
		...
		(function() {
			...
			textbox_x = $el.find('.js-coord-x').textbox({
				type : 'number', value : 0, min : 0, max : 999, hidden_zero : false, tabindex : 1
			}).on('txt:key:enter', function() {
				textbox_y.focus();
			});

			textbox_y = $el.find('.js-coord-y').textbox({
				type : 'number', value : 0, min : 0, max : 999, hidden_zero : false, tabindex : 2
			}).on('txt:key:enter', function() {
			...
			dropdown = $el.find('.dd_coordinates').dropdown({
				list_pos : 'center', hover : false, click : true, type : 'image', value : 0,
				template : 'tpl_dd_map_coordinates', options : settings.options, auto_hide_list : false,
				l10n : settings.l10n, exclude_click_nodes : settings.exclude_click_nodes,
