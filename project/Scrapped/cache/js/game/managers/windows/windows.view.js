	...
	views.WindowsView = Backbone.View.extend({
		...
		initialize : function (options) {
			...
			var namespace = "new_windows_manager",
				...
				onMoveEventName = isiOS ? 'touchmove.' + namespace + ' touchmove.old_windows_manager_ipad' : 'mousemove.' + namespace,
			...
			var dragDropHandler = function(e) {
				...
				if ($target.hasClass('js-window-move') || $target.hasClass('ipad_drag_element')) {
					$document.on(onMoveEventName, function(e) {
						...
						dx = w_offset.left - delegate_target_offset.left + (e.clientX - sx);
						dy = w_offset.top - delegate_target_offset.top + (e.clientY - sy);
		...
		removeWindow : function(model) {
			...
			var cid = model.cid, controller = this.getWindowView(cid);
