...
(function($) {
	...
	model = {
		...
		removePlan : function(plan_id) {
			var plans = this.getPlanList(), i = plans.length;
	...
	view = {
		...
		initializeMainLayout : function() {
			...
			CM.register(cm_context, 'pgr_plan_list', $content.find(".pgr_plan_list").pager({
				activepagenr : at_model.getCurrentPlanListPage(), per_page : this.per_page, total_rows : model.getPlansCount()
		...
		initializePlansList : function(page_nr) {
			...
			$content.find('.game_table .row_plan').each(function(i, el) {
				...
				CM.register(ctx_plan_list, 'btn_open_plan_' + index, $el.find('.ap_caption_name').button({
					template : 'empty', caption : plan.name_short,
					tooltips: [
						{title: plan.name}
					]
				}).on("btn:click", function() {
	...
	controller = {
		initialize : function(obj) {
			...
			ctx_create_plan = {main : cm_context.main, sub : 'wnd_create_plan'};
			ctx_plan_list = {main : cm_context.main, sub : 'plan_list'};
