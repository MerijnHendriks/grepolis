/*global $, Backbone, us, WM, DM, Logger, GameEvents, MM, gpAjax */
...
(function(views, collections, models, gamemodels, gamecollections) {
	...
	function prettyPrintRenderPage(window_model, tab_model, render_type, data, known_data) {
		var rawCreationArguments = [],
			...
			idx, types_length = types.length, type, placeholder,
			elements,
			element_name,
			model_name,
			element,
	...
	models.WindowModel = Backbone.Model.extend({
		defaults : {
			skin : 'wnd_skin_classic', css_class : '',

			max_instances : Infinity,

			//Look
			title : 'Default title',
			width : 650, height : false, minwidth : 150, minheight : 200, maxwidth : null, maxheight : null,
			visible : true,
			...
			resizable : false, closable : true, minimizable : true, minimized : false, focused : false, zindex : 1000,
				...
				gpAjax.ajaxGet('frontend_bridge', 'fetch', {
					...
					success: function(layout, data/*, retdata, flag, t_token*/) {
						/*try {*/
							...
							if (data.templates) {
								DM.loadData({'templates' : data.templates});
		...
		replaceModels : function(models) {
			return this.replaceData({models : models});
		},

		replaceCollections : function(collections) {
			return this.replaceData({collections : collections});
		},
		...
		minimize : function() {
			if (this.isMinimizable()) {
				this.set({minimized : true, visible : false, focused : false});
			}
		},
		...
		manualClose : function() {
			WM.closeWindow(this, {manual_close : true});
		},
		...
		setActivePageNr : function(nr, props) {
			this.set({activepagenr : nr}, props);
		},
		...
		getKnownDataIds : function(tab_model) {
			var known_data = {models : [], collections : [], templates : []},
