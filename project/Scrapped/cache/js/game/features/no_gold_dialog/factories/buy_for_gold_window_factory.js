	...
	var BuyForGoldWindowFactory = {
		...
		openReductUnitBuildTimeForGoldWindow : function(btn, order, callback) {
			...
			var confirmation_event_callback = function() {
				var unit_id = order.unit_id, building_type = order.building_type;

				$.Observer(GameEvents.premium.build_time_reduction).publish({type : 'unit', id : unit_id, place_name : building_type});
			};
		...
		openReductBuildingBuildTimeForGoldWindow : function(btn, order, callback) {
			var confirmation_event_callback = function() {
				...
				$.Observer(GameEvents.premium.build_time_reduction).publish({type: 'building', id : building_type});
			};
		...
		openExtendPowerForGoldWindow : function(btn, casted_power, callbacks) {
			...
			executeConfirmation(btn, cost, callback, window.ConfirmationExtendPowerWindowData);
		},
		...
		openReductResearchBuildTimeForGoldWindow : function(btn, order_data, callback) {
			...
			var confirmation_event_callback = function() {
				...
				$.Observer(window.GameEvents.premium.build_time_reduction).publish({type: 'research', id : research_type});
			};
		...
		openConfirmationBuyEventCurrency: function (btn, amount, cost, callback) {
			...
			executeConfirmation(btn, cost, callback, ConfirmationBuyCurrency, {amount: amount});
		}
	};
