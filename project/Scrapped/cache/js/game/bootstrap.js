...
(function($, window, Game) {
	...
	function setupLayout(models, collections) {
		...
		onFinishedLoadedCallback = function() {
			...
			if (incoming_attack_count > 0) {
				$.Observer(GameEvents.attack.incoming).publish({count: incoming_attack_count});
			}

			//show conquest window if the initial town is being conquered
			var initial_town = collections.towns.getCurrentTown();
			initializeConquestWindow(initial_town);
	...
	function initializeConquestWindow(initial_town) {
	...
	function createModels() {
		...
		var models = {
			heroes : MM.checkAndPublishRawModel('Heroes', {id: Game.player_id, persistent : true}),
			premium_features: MM.checkAndPublishRawModel('PremiumFeatures', {id: Game.player_id, persistent : true}),
			player_settings: MM.checkAndPublishRawModel('PlayerSettings', {id: Game.player_id, persistent : true}),
			player_ledger: MM.checkAndPublishRawModel('PlayerLedger', {id: Game.player_id, persistent : true}),
			player_report_status: MM.checkAndPublishRawModel('PlayerReportStatus', {id: Game.player_id, persistent : true}),
			player_gods: MM.checkAndPublishRawModel('PlayerGods', {id: Game.player_id, persistent : true}),
			quickbar: MM.checkAndPublishRawModel('Quickbar', {id: Game.player_id, persistent : true}),
			phoenician_salesman: MM.checkAndPublishRawModel('PhoenicianSalesman', {id: Game.player_id, persistent : true}),
			current_player: new window.GameModels.CurrentPlayer({
				player_model: MM.checkAndPublishRawModel('Player', {id: Game.player_id, persistent : true}),
				player_ranking_model: MM.checkAndPublishRawModel('PlayerRanking', {id: Game.player_id, persistent : true})
			}),
			unit_time_to_arrival: new GameModels.UnitTimeToArrival(),
			payment_config: MM.checkAndPublishRawModel('PaymentConfig', {id: Game.player_id, persistent : true}),
			instant_buy_data: MM.checkAndPublishRawModel('InstantBuyData', {id: Game.player_id, persistent : true}),
			inventory : MM.checkAndPublishRawModel('Inventory', {id: Game.player_id, persistent : true}),
			map_chunks: MM.checkAndPublishRawModel('MapChunks', {id: Game.player_id, persistent : true}),
			mermaid : MM.checkAndPublishRawModel('Mermaid', {id: Game.player_id, persistent : true}),
			maintenance : MM.checkAndPublishRawModel('Maintenance', {id: 1, persistent : true}),
			player_killpoints: MM.checkAndPublishRawModel('PlayerKillpoints', {id: Game.player_id, persistent : true}),
			daily_login : MM.checkAndPublishRawModel('DailyLoginBonus', {id: Game.player_id, persistent : true}),
			grepo_score : MM.checkAndPublishRawModel('GrepoScore', {id: Game.player_id, persistent : true}),
			player_attack_spot : MM.checkAndPublishRawModel('PlayerAttackSpot', {id: Game.player_id, persistent : true}),
			survey : MM.checkAndPublishRawModel('Survey', {id: Game.player_id, persistent : true}),
			mission_status: MM.checkAndPublishRawModel('MissionStatus', {id: Game.player_id, persistent : true}),
			mission_report: MM.checkAndPublishRawModel('MissionReport', {id: Game.player_id, persistent : true}),
			town_id_list: MM.checkAndPublishRawModel('TownIdList', {id: Game.player_id, persistent : true})
		};

		if (Features.isCasualWorld()) {
			models.casual_worlds_blessed_town = MM.checkAndPublishRawModel('CasualWorldsBlessedTown', {id: Game.player_id, persistent: true});
		}

		if (Features.isOlympusEndgameActive()) {
			models.olympus = MM.checkAndPublishRawModel('Olympus', {id: 1, persistent: true});
		}
	...
	function createTownAgnosticCollections() {
		return {
			trades : new window.TownAgnosticCollection(
					[],
					{segmentation_keys: ['destination_town_id', 'origin_town_id'], model_class: 'Trade', fragment_constructor:  window.GameCollections.Trades}
			),
			unit_orders : new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'UnitOrder', fragment_constructor:  window.GameCollections.RemainingUnitOrders}
			),
			casted_powers : new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'CastedPowers', fragment_constructor:  window.GameCollections.CastedPowers}
			),
			units : new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'current_town_id', model_class: 'Units', fragment_constructor:  window.GameCollections.Units}
			),
			supporting_units : new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'home_town_id', model_class: 'Units', fragment_constructor:  window.GameCollections.ActivePlayerSupportsTowns}
			),
			building_orders: new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'BuildingOrder', fragment_constructor: window.GameCollections.BuildingOrders}
			),
			research_orders: new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'ResearchOrder', fragment_constructor: window.GameCollections.ResearchOrders}
			),
			town_buildings: new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'Buildings', fragment_constructor: window.GameCollections.TownBuildings}
			),
			town_researches: new window.TownAgnosticCollection(
					[],
					{segmentation_key: 'town_id', model_class: 'Researches', fragment_constructor: window.GameCollections.TownResearches}
			),
			celebrations: new window.TownAgnosticCollection(
				[],
				{segmentation_key: 'town_id', model_class: 'Celebration', fragment_constructor: window.GameCollections.Celebrations}
			),
			movements_spys: new window.TownAgnosticCollection(
				[],
				{segmentation_key: 'home_town_id', model_class: 'MovementsSpy', fragment_constructor: window.GameCollections.MovementsSpys}
			),
			...
			movements_conquerors: new window.TownAgnosticCollection(
				[],
				{segmentation_key: 'home_town_id', model_class: 'MovementsConqueror', fragment_constructor: window.GameCollections.MovementsConquerors}
			),
			movements_colonizations: new window.TownAgnosticCollection(
				[],
				{segmentation_key: 'home_town_id', model_class: 'MovementsColonization', fragment_constructor: window.GameCollections.MovementsColonizations}
			)
		};
	}
	/**
	...
	function createCollections() {
		var collections = {
			...
			finished_wonders : new window.GameCollections.FinishedWonders([], {alliance_id: Game.alliance_id}),
	...
	function estimateStartupTime() {
		...
		gpAjax.ajaxPost('debug', 'log_startup_time', {log_startup_time: log_startup_time}, false, function(data) {
			if (data.t && data.t > 0) {
