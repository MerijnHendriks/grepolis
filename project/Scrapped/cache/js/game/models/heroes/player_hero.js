...
//			}
	...
	PlayerHero.levelUpHero = function(coins) {
		this.execute('levelUpHero', {type: this.getId(), amount: coins});
	};
	...
	PlayerHero.halveCureTime = function() {
		this.execute('halveCureTime', {type: this.getId()});
	};
	...
	PlayerHero.healInstant = function() {
		this.execute('healInstant', {type: this.getId()});
	};
	...
	PlayerHero.assignToTown = function(callbacks) {
		this.execute('assignToTown', {type : this.getId(), target_town_id: Game.townId}, callbacks);
	};

	PlayerHero.unassignFromTown = function() {
		this.execute('unassignFromTown', {type : this.getId(), target_town_id: this.getOriginTownId()});
	};

	PlayerHero.cancelTownTravel = function(callbacks) {
		this.execute('cancelTownTravel', {type : this.getId(), target_town_id: this.getOriginTownId()}, callbacks);
	};
