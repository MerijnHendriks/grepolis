...
(function($) {
	...
	$.fn.numberChangeIndicator = function(params) {
		var settings = $.extend({
			...
			styles_increase : {color : '#8dc63f', fontWeight : 'bold'},
			styles_decrease : {color : '#ef3100', fontWeight : 'bold'},
			styles_to_animate : {'fontSize' : 30, 'opacity' : 0},
		...
		function animateChange(styles, value) {
			...
			if ($relate_to) {
				...
				pos = {top : pos.top - relative_to_pos.top, left : pos.left - relative_to_pos.left};
			}
			...
			$anim.appendTo($append_to).css($.extend({
				position : 'absolute', zIndex : 100000, top : pos.top, left : pos.left, width : $caption.width()
			...
			$anim.animate(settings.styles_to_animate, {
				step : function() {
					...
					if (settings.center_hor) {
						$anim.css({
							marginLeft : -1 * ($anim.outerWidth(true) - width) / 2
						});
					}
					...
					if (settings.center_ver) {
						$anim.css({
							marginTop : -1 * ($anim.outerHeight(true) - height) / 2
						});
					}
				},
