	...
	$.fn.scroller = function(params) {
		...
		function renderItems(force) {
			var i, id, el, page_offset = settings.page_offset,
				items_count = settings.items_count, item_height = settings.item_height,
		...
		function calculateEdgeItems() {
			var scroll_top = $scroll_event_keeper.scrollTop(),
				...
				items = settings.items, i, l = items.length,
				pos = 0, visible_first, visible_last,
			...
			return {first_item : visible_first, last_item : visible_last};
		}

		function checkForSelected() {
			var items = settings.items, i, l = items.length, counter = 0;
		...
		function initializeScrollSimulator() {
			...
			$el.find('.scroll_simulator').css({height : size});
		}
		...
		function reset(dont_scroll) {
			...
			var i = free_hepler_rows.length, row;
