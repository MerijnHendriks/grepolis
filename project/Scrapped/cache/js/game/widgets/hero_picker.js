...
(function($) {
	...
	$.fn.heroPicker = function(params) {
		...
		var _self = this,
			$el = $(this), $list, $icon, $caption, $picker,
		...
		function registerComponents() {
			...
			hero_dropdown = $list.heroDropdown(us.extend({}, settings, {tooltips: tooltips.dropdown}));

			$btn_add = $btn_add.button({
				tooltips: tooltips.btn_add || [{title: l10n.assign}]
			});

			if(settings.should_have_remove_and_change_btn) {
				$btn_change = $btn_change.button({
					disabled: true,
					tooltips: tooltips.btn_change || [{title: l10n.change}]
				});

				$btn_remove = $btn_remove.button({
					disabled: true,
					tooltips: tooltips.btn_remove || [{title: l10n.unassign}]
				});
			}
		...
		this.getCurrentlySelectedHeroAndLevel = function() {
			 return { name: hero_dropdown.getValue(), level: $level_spinner.getValue() };
		};
