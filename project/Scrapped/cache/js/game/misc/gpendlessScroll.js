...
(function() {
	...
	function GPEndlessScroll(args) {
		...
		$(document).unbind('es_jump_' + args.elem_id); // unbind all trigger first
		...
		var that = this,
			es_object = {
				...
				callback: function(isScrollDown){
					...
					return {visible_height: $('#' + args.elem_id).height(), inner_elem_id: args.insert_elem_id, pagination_elem_id: es_pagination_id}; // TODO: do we need this??
				}
			};
		...
		this.getData = function(ls_key, ls_store_key, new_start_element, scroll_direction) {
			LocalStore.get(ls_key, function(ok, data){
				if(ok) {
					...
					if(data.length !== 0){
						var scroll_options = {start_element : new_start_element, elements_length : $(data).length, view : data};
					...
					ajax_data = {start_element: new_start_element, limit: args.limit, scroll_direction: 'top', refetch: 1, es_args : args.es_args};
