	...
	var LayoutCityConstructionOverlay = Class.extend({
		...
		registernormalModeEventsListeners : function() {
			...
			this.listenToMultiEvents('all_events_normal_mode_to_force_update', [
				...
				{obj : this.getCollection('building_orders'), method : 'onOrderPropertyChange'},
				{obj : this.getCollection('remaining_unit_orders'), method : 'onOrderCountChange'},
				{obj : this.getCollection('remaining_unit_orders'), method : 'onOrderPropertyChange'},
				{obj : this.getCollection('research_orders'), method : 'onOrderCountChange'}
			], rerenderPage);
		},
		...
		registerConstructionModeEventsListeners : function() {
			...
			this.listenToMultiEvents('all_events_construction_mode_to_force_update', [
				...
				{obj : this.getModel('premium_features'), method : 'onCuratorChange'}
			], rerenderPage);
		},
		...
		renderPage : function() {
			...
			//but we have to call it after we load the data from the server
			return this;
		},
		...
		getConstructedBuildings : function() {
			...
			us.each(this.getNormalBuildings(), function(building_level, building_id) {
				...
				if (is_construction_mode) {
					if (!this.isBuildingRequirementsFullfilled(building_id)) {
		...
		isSpecialBuildingSpotPossible : function(slot_buildings_ids) {
			...
			for (var i = 0, l = slot_buildings_ids.length; i < l; i++ ) {
				...
				if (this.isBuildingRequirementsFullfilled(building_id) || this.areSpecialTutorialQuestsRulesApplied(building_id)) {
					return true;
				}
			}
		 ...
		 * @return {Boolean}
		 */
		isBuildingRequirementsFullfilled : function(building_id) {
			var building_orders = this.getCollection('building_orders');

			return GameDataBuildings.isBuildingRequirementsFullfilled(building_orders, this.current_town_model, building_id);
		},
		...
		showBlockingLayer : function() {
			//@see this._render why we need to check for the view
			if (this.view) {
				this.view.showBlockingLayer();
			}
		},

		hideBlockingLayer : function() {
			//@see this._render why we need to check for the view
			if (this.view) {
				this.view.hideBlockingLayer();
			}
		},
