...
define('factories/tooltip_factory', function() {
	...
	function getResourcesForBuildBuilding(building, ceil, modifier) {
		...
		if (ceil) {
			requirements = {
				wood : {amount : Math.ceil(building.needed_resources.wood * modifier)},
				stone : {amount : Math.ceil(building.needed_resources.stone * modifier)},
				iron : {amount : Math.ceil(building.needed_resources.iron * modifier)},
				population : {amount : building.pop}
			};
		} else {
			requirements = {
				wood : {amount : parseInt(building.needed_resources.wood * modifier, 10)},
				stone : {amount : parseInt(building.needed_resources.stone * modifier, 10)},
				iron : {amount : parseInt(building.needed_resources.iron * modifier, 10)},
				population : {amount : building.pop}
			};
		}
	...
	function getNeededResourcesInfo(town_id, building, options) {
		...
		var result = '', itown = ITowns.getTown(town_id),
			resources = itown.resources(), current_production = itown.getProduction(),
			time_to_build = 0,
			enough_population = true, enough_resources = true, upgrade_not_possible = options.upgrade_not_possible || false,
	...
	function getOlympusNextJumpAtTimestamp() {
	...
	function isGlobalShieldActive() {
		var OlympusHelper = require('helpers/olympus');

		return OlympusHelper.getOlympusModel().isGlobalShieldActive();
	}

	function getNextGlobalShieldToggle() {
		var OlympusHelper = require('helpers/olympus');

		return DateHelper.timestampToDateTime(OlympusHelper.getOlympusModel().getNextGlobalShieldToggle());
	}
	...
	var TooltipFactory = {
		...
		getBasicPowerTooltipWithoutImage : function(power_id) {
			return this.createPowerTooltip(power_id, {show_image : false});
		},

		getPowerTooltipWithCountdown : function(power_id, configuration, casted_power_end_at) {
			return this.createPowerTooltip(power_id, {casted_power_end_at: casted_power_end_at}, configuration);
		},

		getPowerTooltipWithDefaultSettings : function(power_id, configuration) {
			return this.createPowerTooltip(power_id, {show_costs : true}, configuration);
		},
		 ...
		 * @return {String}
		 */
		createPowerTooltip: function(power_id, properties, configuration_arg, progress) {
			var configuration = configuration_arg || {};
			var level = getPowerLevel(configuration);
			var gd_power = GameData.powers[power_id];
			var tooltip_data = GameDataPowers.getTooltipPowerData(gd_power, configuration, level, progress);
		...
		getBuildingConstructionRequirements : function(town_id, building, full_queue, show_description) {
			var result = '',
				max_level_reached = false, upgrade_not_possible = false;
			...
			if (building.max_level) {
				...
				res_info = getNeededResourcesInfo(town_id, building, {upgrade_not_possible : upgrade_not_possible});
		...
		getHeroCard: function(hero_id, options) {
			var hero_card = DM.getTemplate('heroes', 'hero_card'),
				hero_data = us.extend({portrait: hero_id}, GameData.heroes[hero_id]),
				settings = us.extend({hero: hero_data, l10n: l10n.hero_card}, options);
		...
		getRefundTooltip : function(resources) {
			var html = '', res_id, path = Game.img(),
				gd_resources = GameData.resources, separator = ' ';
		...
		getOlympusTempleTooltip: function (temple) {
			...
			if (!isGlobalShieldActive() && !is_olympus) {
				result += '<br /><br /><b>' + l10n.next_shield_toggle(getNextGlobalShieldToggle()) + '</b>';
			}

			if (is_olympus) {
				result += '<br /><br /><b>' + l10n.next_jump(getOlympusNextJumpAtTimestamp());
			}
