...
(function() {
	...
	var PopupFactory = {
		...
		init : function() {
			...
			var pfs = [
				{i:'curator',t:_('Administrator'),d:PopupFactory.texts.curator_info},
				{i:'trader',t:_('Merchant'),d:PopupFactory.texts.trader_info},
				{i:'priest',t:_('High Priestess'),d:PopupFactory.texts.priest_info},
				{i:'commander',t:_('Commander'),d:PopupFactory.texts.commander_info},
				{i:'captain',t:_('Captain'),d:PopupFactory.texts.captain_info}
			];
			var i = pfs.length;
			while (i--) {
	...
	jQuery.fn.extend({
		setPopup: function(popup_type) {
			var details_pos = popup_type.indexOf('_details'),
				contains_details = details_pos !== -1;

			if (true && contains_details) {
				this.tooltip(TooltipFactory.getUnitCard(popup_type.substr(0,details_pos)), {}, false);
			} else {
