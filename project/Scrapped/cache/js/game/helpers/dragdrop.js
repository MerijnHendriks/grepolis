...
(function($) {
	...
	var HelperDragDrop = {
		...
		getLimitedPosition : function(
			...
			var left_edge = 0,
				top_edge = 0,
				right_edge = viewport_width - draggable_width * zoom_factor,
				bottom_edge = viewport_height - draggable_height * zoom_factor,
			...
			return {
				x : draggable_is_wider_than_viewport ? Math.max(Math.min(left_edge, x), right_edge) : draggable_offset.left,
				y : draggable_is_higher_than_viewport ? Math.max(Math.min(top_edge, y), bottom_edge) : draggable_offset.top
			};
		},
		...
		getDragDropEventHandler : function($viewport, namespace, drag_occured_callback, drag_stopped_callback) {
			...
			return function(e) {
				...
				var delegate_target_offset = $delegate_target.offset() || {left : 0, top : 0};
				...
				var transform_origin = $draggable_layer
						.css('transformOrigin').split(' ').map(function(el){ return parseInt(el,10); }),
				...
				$document.on(onMoveEventName, function(e) {
                    ...
                    var x = draggable_offset.left - delegate_target_offset.left + (event.clientX - mouse_start_x),
