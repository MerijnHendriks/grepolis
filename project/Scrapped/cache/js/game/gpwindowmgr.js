...
(function() {
	...
	window.GPWindowMgr = function() {
		...
		this.Create = function (type, title, UIopts) {  // Variadic Args..
			...
			// but only kill other windows when the 'to open' window isnt important
		...
		this.closeAll = function () {
			var wndstack = [], t = TypeInfo.length, w, i;
		...
		/**
		 * Returns the number of openened windows by type:
		 * @deprecated
		 */
		 ...
		 *
		 * @param {WndHandlerDefault}
		 */
		this.focusWindow = function(wndhandler) {
			if (typeof WM === 'object') {
				WM.giveFocus(wndhandler);
			}
		};
		...
		this.getOpenedClosableWindows = function() {
			var window, windows = wndStack, i, l = windows.length,
				closable = [];
		...
		/**
		 * Returns an array of the openened widnows by type:
		 * @param type Number
		 *
		 */
	...
	GPWindowMgr.extendLayoutWithShortLinks = function(context) {
		...
		context.allianceProfile = function() {
			this.open = function(alliance_name, alliance_id) {
				...
				if (!w) {
					context.wnd.Create(context.wnd.TYPE_ALLIANCE_PROFILE, alliance_name, {alliance_id: alliance_id});
				} else {
					w.toTop();
					w.requestContentGet('alliance', 'profile', {alliance_id: alliance_id});
		...
		context.playerProfile = function() {
			this.open = function(player_name, player_id) {
				...
				if (!w) {
					context.wnd.Create(context.wnd.TYPE_PLAYER_PROFILE, title, {player_id: player_id});
				} else {
					w.toTop();
					w.requestContentGet('player', 'get_profile_html', {player_id: player_id});
					w.setTitle(title);
				}
			};
		};
		...
		context.dialogWindow = {
			open: function(content, title, width, height, onClose, minimizable, UIopts) {
				var opts = {onClose: onClose, minimizable: minimizable},
		...
		context.createApplication = function() {
			this.open = function(alliance_id) {
				...
				if (!w) {
					...
					w.toTop();
					w.requestContentGet('alliance', 'create_application', {alliance_id: alliance_id});
				}
			};
		};
