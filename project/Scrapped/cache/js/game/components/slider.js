...
(function ($) {
	...
	$.fn.slider2 = function (params) {
		...
		var _self = this,
			$document = $(document), $el = $(this),
			$slider_handle_container, $slider_handle,
		...
		function setValue(value, props) {
			...
			var is_disabled = isDisabled();

			var silent = props.silent || false,
				step = getStepValue(),
		...
		function bindEvents() {
			...
			$el.on(mousedown_event_name, '.js-slider-handle', function(e) {
				var $target = $(e.currentTarget), target_pos, stopPos, startPos;
				...
				$document.on(mousemove_event_name, us.throttle(function(e) {
					...
					if (settings.live) {
						setValue(null, {stopPos : stopPos, silent : !settings.live});
					}
			...
			$slider_handle_container.on('click.slider', function(e) {
				...
				if (settings.select_by_click && !$target.hasClass('js-slider-handle')) {
					setValue(null, {stopPos : e[currpos_method_name] - pos[pos_method_name]});
				}
			});
		}
