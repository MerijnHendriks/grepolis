	...
	var SoundController = Backbone.Model.extend({
		...
		initialize: function(params) {
			...
			this.set("categories_enabled", {
				...
				effects: params.sound_effects && !Game.isiOs(),
				click: params.click_sounds && !Game.isiOs()
			});
			...
			this.on("change:sound_volume", function() {
				this.get("categories").forEach(function(item) {
					item.trigger("update_volume", {global: self.getVolume()});
				});
			});
		...
		/**
		 * Preload a random backgound sound and push it to the background-sound queue
		...
		playBackground: function() {
			...
			var played_bg = this.get("played_bg"), element;
			if (played_bg === 2) {
		...
		createCategory: function(name) {
			var category = new SoundCategory({name: name}),
				self = this;
			this.get("categories").add(category);

			category.on("change:volume", function() {
				category.trigger("update_volume", {global: self.getVolume()});
			});
			return category;
		},
		enabled: function() {
		...
		enable: function(config) {
			...
			var category = us.first(this.get("categories").where({name: config.category_name})),
				model;
		...
		enableSound: function(sound_def) {
			...
			/*Logger.get("gameSounds").log(function(){
				return [ "SoundController::enableSound", defaultVals];
			});*/
		},
		...
		canPlay: function() {
			return (!Game.isiOs() || !this.get("isiOsPlaying"));
		},
		play: function(sound_def) {
			if (!this.enabled() || !this.canPlay()) {
				return;
			}

			if (sound_def) {
				if (sound_def.player) {
					this.iOsStartPlaying();
		...
		iOsStartPlaying: function() {
			this.set("isiOsPlaying", false);
		},
		iOsStopPlaying: function() {
			this.set("isiOsPlaying", false);
		},
		iOsPlayBackground: function() {
			if (this.get("background_playing")) {
				return;
			}
			this.set("background_playing", true);
			this.playBackground();
		},
