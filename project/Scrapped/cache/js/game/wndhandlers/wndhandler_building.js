	...
	function WndHandlerBuilding(wndhandle){
		...
		this.registerEventListeners = function() {
			...
			switch (this.currentBuilding) {
				...
				//FALLTHROUGH
				case 'docks':
					// the case above has no break. both the barracks and the docks do essentially the same. be carefull
					//
					// Inject the window_id into the DOM to have it accessable for 'subviews' to avoid w()
					this.wnd.getJQElement().find('.unit_order_task a').data('wnd_id', this.wnd.getIdentifier());
					this.wnd.getJQElement().find('#unit_order_info #unit_order_input').data('wnd_id', this.wnd.getIdentifier());
					...
					$.Observer(GameEvents.town.units.order.done).subscribe([name], reloadBuildingWithUnitOrders);
...
// re-register eventListeners
	...
	WndHandlerBuilding.prototype.registerBuildingPlaceComponents = function() {
		...
		var context = this.wnd.getContext(),
			celebration_cost = GameData.celebration_cost;
	...
	WndHandlerBuilding.prototype.registerFightSimulatorHeroComponents = function() {
		...
		var l10n = DM.getl10n('place', 'simulator'),
			all_heroes = [{info: l10n.unassign, value: ''}];
		...
		var att_units = FightSimulator.player_units.att,
			def_units = FightSimulator.player_units.def,
