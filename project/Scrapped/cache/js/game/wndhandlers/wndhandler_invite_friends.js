(function() {
	...
	function WndHandlerInviteFriends(wndhandle) {
		...
		/**
		 * start the procedure for the autoselected spot
		 */
		function selectSpotAuto(spot) {
			var MapHelpers = require('map/helpers');
			var spotPixels = MapHelpers.map2Pixel(spot.x, spot.y);
			HelperTown.townSwitch(spot.town_id);
			...
			WMap.centerMapOnPos(spotPixels.x, spotPixels.y, true);
		...
		this.clickHandler = function (e) {
			...
			switch ((type = href[0])) {
				case 'getReward':
					that.wnd.requestContentPost(that.controller, 'reward', {mentee_id: parseInt(href[1], 10)});
					break;
				case 'accept':
	...
	WndHandlerInviteFriends.prototype.registerComponents = function() {
		var context = this.wnd.getContext(), root = this.wnd.getJQElement(), l10n = DM.getl10n('invite_friends', 'invite_friends'), _self = this;
	...
	WndHandlerInviteFriends.prototype.inviteByBanner = function() {
		...
		this.wnd.requestContentGet(this.controller, 'banners', {path: path});
	};
	...
	WndHandlerInviteFriends.prototype.onRcvData = function (data, controller, action) {
		...
		if (this.wnd.getJQElement().find('#txt_invitee_email').length) {
			...
			CM.register(context, 'txt_invitee_email', mail_credentials.invitee_email.textbox({value: mail_credentials.invitee_email.attr('value')}).removeAttr('value'));
			CM.register(context, 'txt_invitee_username', mail_credentials.invitee_username.textbox({value: mail_credentials.invitee_username.attr('value')}).removeAttr('value'));
			CM.register(context, 'txt_real_invitor_name', mail_credentials.real_invitor_name.textbox({value: mail_credentials.real_invitor_name.attr('value'), type: 'custom', live: true, regexp: /^.{0,40}$/, invalidmsg: msg_too_long_err.replace('%n', '40')}).removeAttr('value'));
			CM.register(context, 'txt_real_invitee_name', mail_credentials.real_invitee_name.textbox({value: mail_credentials.real_invitee_name.attr('value'), type: 'custom', live: true, regexp: /^.{0,40}$/, invalidmsg: msg_too_long_err.replace('%n', '40')}).removeAttr('value'));
			CM.register(context, 'txta_message', mail_credentials.message.textarea({value: mail_credentials.message.attr('value'), maxlength: 160, invalidmsg: msg_too_long_err.replace('%n', '160')}).removeAttr('value'));
		...
		$('div#invite_data select#isle_selector').bind('focus', function (event) {
			$('div#invite_data input').removeAttr('checked');
	...
	WndHandlerInviteFriends.prototype.acceptSpot = function (selectionType, spot) {
		...
		if (this.inviteType == 'facebook') {
			// if invitation is for facebook
			// we trigger popup instead of reading the new layout by ajax
			// this popup tunnels through our facebook invite action and redirects
			// to the facebook share url
			var spot_url_details = '';
			$.each(spot, function (k, v) {
				spot_url_details += '&spot[' + k + ']=' + v;
			});

			var fb_url = url(this.controller, 'fbinvite', {
				type : this.inviteType,
				selection : this.selectionType,
				path: path
			});

			window.open(fb_url + spot_url_details,
				'facebookInvite',
	...
	WndHandlerInviteFriends.selectSpotOnMap = function (data) {
		...
		var wnd = GPWindowMgr.getOpenFirst(GPWindowMgr.TYPE_INVITE_FRIENDS);
	...
	/**
	 * end the procedure for the autoselected spot
	 */
	...
	WndHandlerInviteFriends.prototype.showBanner = function (banner_type) {
		...
		if (!banner_type.match(/^\d{2,3}x\d{2,3}$/)) {
			...
			if (height > 125) {
				...
				$('#banner_info').css({'float': 'left', 'margin-left': '20px'});
				input_width = input_width - real_width - 20;
			} else {
				$('#banner_preview').css('float', 'none');
				$('#banner_info').css({'float': 'none', 'margin-left': '0'});
			}
			...
			$('#banner_bbcode')
				...
				.width(input_width);
			$('#banner_html')
				.val('<a href="' + this.referer.html + '"><img src="' + img_url + '"/></a>')
				.width(input_width);
		}
	};
