...
(function() {
	...
	var RemainingUnitOrders = function() {}; // never use this, becasue it will be overwritten

	RemainingUnitOrders.model = UnitOrder;
	RemainingUnitOrders.model_class = 'UnitOrder';

	RemainingUnitOrders.initialize = function(models, options) {
		this.comparator = function(model) {
			//We are comparing by ID because we want to keep the same order as the orders were created.
			//If last order is instantly bought it should not go to the beginning due to the change of created_at and completed_at
			return model.id;
		};
		...
		this.on('remove', function(unit_order) {
			$.Observer(GameEvents.town.units.order.done).publish(unit_order);
		});
		...
		//if (options.created_as_current_collection) {
			...
			//The condition was commented out because mass recruit operates on all unit order collections, so timers have to ran for all of them
