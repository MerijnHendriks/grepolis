...
define('alliance/collections/alliance_pacts', function() {
	...
	AlliancePacts.getAllianceIdsByRelation = function(relation) {
		return this.where({ relation : relation });
	};

	AlliancePacts.isInPeacePact = function(alliance_id) {
		var pact_info = this.findWhere({ alliance_1_id : alliance_id, relation : 'peace' });
		if (!pact_info) {
			pact_info = this.findWhere({ alliance_2_id : alliance_id, relation : 'peace' });
		}
		return (pact_info) ? true : false;
	};

	AlliancePacts.isInWarPact = function(alliance_id) {
		var pact_info = this.findWhere({ alliance_1_id : alliance_id, relation : 'war' });
		return (pact_info) ? true : false;
	};
	...
	AlliancePacts.isAllianceWithCurrentAllianceInPeacePact = function(alliance_id) {
		...
		if (alliance_id !== Game.alliance_id) {
			peace_pact = this.findWhere({ alliance_1_id : alliance_id, relation : 'peace' });
			if (!peace_pact) {
				peace_pact = this.findWhere({ alliance_2_id : alliance_id, relation : 'peace' });
			}
		}
		return peace_pact;
	};
	...
	AlliancePacts.isAllianceWithCurrentAllianceInWarPact = function(alliance_id) {
		return this.findWhere({ alliance_2_id : alliance_id, relation : 'war' });
	};
