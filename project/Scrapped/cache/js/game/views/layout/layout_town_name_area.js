/*global us, BuyForGoldWindowFactory, TooltipFactory, HelperTown, GameEvents, NotificationLoader, TM */
...
(function() {
	...
	var LayoutTownNameArea = BaseView.extend({
		...
		registerTownNameAreaComponents : function() {
			var _self = this, controller = this.controller, $el = this.$el;

			controller.registerComponent('btn_prev_town', $el.find('.btn_prev_town').button({
				template : 'empty', disabled : controller.isTownSwitchPossibleInCurrentGroup()
			...
			controller.registerComponent('btn_toggle_town_groups_menu', $el.find('.btn_toggle_town_groups_menu').singleDoubleClick({
			}).on('sdc:click', function() {
			...
			}).on('rn:rename:stop', function(e, _rn, new_town_name, old_town_name) {
